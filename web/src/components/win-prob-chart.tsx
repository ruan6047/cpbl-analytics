...
type WpPoint = { evt: string | null; inning: number | null; half: string | null;
  hitter: string | null; away: number; home: number; wp: number };

export function WinProbChart({ sno, kind, year, homeName, awayName, homeColor }: {
  sno: number; kind: string; year?: number;
  homeName: string; awayName: string; homeColor: string;
}) {
  const [items, setItems] = useState<WpPoint[] | null>(null);
  useEffect(() => {
    detail.winprob(sno, kind, year).then((d) => setItems(d.items)).catch(() => setItems([]));
  }, [sno, kind, year]);
    ...
    <div className="rounded-xl border border-line bg-surface p-4">
      <div className="mb-1 flex items-baseline justify-between">
        <div className="text-sm font-semibold">
          勝率變化 <span className="text-xs font-normal text-faint">（逐打席推算・{homeName} 視角）</span>
        </div>
