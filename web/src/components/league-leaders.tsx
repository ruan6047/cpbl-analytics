...
import { Card, ENTITY_LINK, Eyebrow, TeamLogo } from "@/components/ui";
              ...
              <div>
                ...
                {top1 ? (
                  <div className="mb-3 flex items-center justify-between border-b border-line pb-2">
                    {/* §3.5：只有球員名文字進連結並帶常駐底線；logo 與隊名副標留在連結外。 */}
                    <div className="flex items-center gap-2">
                      <TeamLogo code={null} name={top1.team} size={28} decorative />
                      <div className="min-w-0">
                        <Link
                          href={`/players/${top1.player_id}`}
                          className={`inline-block max-w-[80px] truncate align-bottom text-sm font-bold ${ENTITY_LINK}`}
                        >
                          {top1.name}
                        </Link>
