...
> **多 AI 協作前先讀 [`docs/AI_WORKFLOW.md`](docs/AI_WORKFLOW.md)**（stub → canonical `.ai-workflow/AI_WORKFLOW.md`，submodule）：階段職責、分支制、部署閘門與留痕。**任務狀態的唯一事實來源＝GitHub Issues＋[user Project #4「cpbl-analytics 任務看板」](https://github.com/users/ruan6047/projects/4)**，寫入一律經 ai-workflow 的 `wfcli`；[`docs/TASKS.md`](docs/TASKS.md) 已於 2026-08-04 cutover **封存唯讀**，只供歷史查閱，不得當作活卡 Ledger 讀或寫。模型能力選擇的唯一來源是 [`docs/MODEL_ROUTING.md`](docs/MODEL_ROUTING.md)，資料庫寫入契約見 [`docs/DATABASE_CONTRACT.md`](docs/DATABASE_CONTRACT.md)。
