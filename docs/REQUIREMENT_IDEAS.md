...
- **未結清賽果的維運訊號**：官方已宣告完賽、本站結果卻尚未結清時，維運者是否需要能區分「待處理」、「確實沒有」與「來源不可用」的訊號？既有每日摘要已提供逐場狀態，但尚未確認誰需要彙整訊號、收到後如何處置；舊卡指定的端點與新模組不是需求前提。來源：[舊 #127](https://github.com/ruan6047/cpbl-analytics/issues/127)。
- **每日鏈日期界線的殘餘風險**：目前每日鏈走已設為台北時區的共用連線，舊 #158 所述的當前漏抓未重現；若有連線池外查詢或借用者更改連線時區，無參數完成場判準仍可能依環境漂移。先確認是否存在實際受影響路徑，再決定是否需要處理；原五處逐一改寫方案不直接沿用。來源：[舊 #158](https://github.com/ruan6047/cpbl-analytics/issues/158)、[已完成 #159](https://github.com/ruan6047/cpbl-analytics/issues/159)。
