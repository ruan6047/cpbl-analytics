...
|---|---|---|---|---|---|---|---|---|---|
| [GAME-RECAP-WP-API1](tasks/GAME-RECAP-WP-API1.md) | INIT-GAME-RECAP | T3 | WP／WPA 參考資訊 API（揭露語意） | 待部署驗證（需部署卡，✅已驗證前不得 release） | `ai/fable-5/GAME-RECAP-WP-API1 @ .claude/worktrees/game-recap-wp-api1-execution（保留至部署驗證後結案）` | 1 | 📦已合併 | ⏸未部署 | 2026-07-27T02:23:52+08:00 |
...
| [ML-OUTCOME-LEAK1](tasks/ML-OUTCOME-LEAK1.md) | — | T3 | 賽果預測先發特徵前視洩漏修正 | 待指派 | — | 0 | 📥Backlog | ⏸未部署 | 2026-07-27T01:29:31+08:00 |
...
| [UX-WP-DISCLOSURE1](tasks/UX-WP-DISCLOSURE1.md) | — | T3 | 賽況頁 WP 曲線誠實註記 | 待部署驗證（需部署卡，✅已驗證前不得 release） | `ai/fable-5/UX-WP-DISCLOSURE1 @ .claude/worktrees/ux-wp-disclosure1-execution（保留至部署驗證後結案）` | 1 | 📦已合併 | ⏸未部署 | 2026-07-27T02:23:52+08:00 |
...
- `INIT-GAME-RECAP` 的資料紅線主鏈：`GAME-RECAP-DATA1 ✅ → GAME-RECAP-PA1 ✅ → GAME-RECAP-WP-VAL1 ✅（全 scope unsupported）→ GAME-RECAP-WP-CAL1 🏁（事後校準 No-Go）→ GAME-RECAP-WP-STRENGTH1（🔍待查核；交付結論 No-Go：賽前特徵時間外無增量資訊）→ GAME-RECAP-WP-API1（**已解阻塞**：2026-07-27 需求方定位改寫 canonical→參考資訊＋揭露〔T3〕，統計改善鏈封存為升級路徑）→ UX-GAME-RECAP1 → UX-GAME-PA1`；首頁 v1 另走 `API-DAILY-SUMMARY1 + UX-OUTCOME-HOME → UX-GAME-HOME1`，不依賴 WPA。
