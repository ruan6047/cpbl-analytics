...
- **範圍 v3（07-13 重規劃；取代 07-12 修訂版）**：
  ...
  4. **逐球球種簡化：卡特併入滑球**（新）：樣本累積前先視為同球種。改法：後端 `tracking.py` `PT_EXPR` 輸出層 alias（卡特→滑球）單點收斂，arsenal/quality_by_pt/配球傾向/位移/球種篩選全數自動繼承並 usage 加總；DB 欄 `pitch_type_pred_v2` **不動**（可逆，之後資料夠再拆回）；前端 `PT_ORDER`/`PITCH_ALIAS` 同步移除卡特；顯示處標注「滑球（含卡特）」。**橫掃維持獨立**（本次僅卡特/滑球；滑曲球→曲球 alias 既有不動）
...
- Log：
  - 07-12 首輪還原＋需求校正（氣泡否決）；07-13 ruan6047 補四項回饋（雷達無說明/本季成績排版+PR 融入/卡特併滑球/熱區區塊名稱與指標）→ Fable 重規劃範圍 v3
