...
| TEAM-HIST1 | 隊史年表補缺（歷年教練團／球隊異動／二軍獎項） | ruan6047 | Opus-4.8@Claude Code | Opus-4.8@Claude Code | 待指派（≠執行者） | `ai/opus-4.8/TEAM-HIST1` | ⚪ | 🔍待查核（182 頁；教練 1,010 筆/284 人、異動 1,397、二軍獎項 18；**冠軍教練第三方驗證 15/15 相符**） |
...
- 狀態：🔍待查核　分支：`ai/opus-4.8/TEAM-HIST1`
- **⚠️ 開卡假設被實測推翻（必讀）**：原判「歷年改名是唯一無替代來源的缺口，優先級最高」——**錯**。
  逐頁掃描 182 個年度頁：有「球隊異動」區塊 156 頁，但**只有 3 頁真的寫了「更改姓名」**（共 7 筆，
  且全部解析成功、零遺漏）。twbsball 隊史**沒有系統性記錄改名**。
  **真正的權威來源是官方登錄名單（team_roster）＋ gamelog**：實測發現 `players` 有 6 位現役球員名字
  過期（朱迦恩／魔力藍／王浩原／冉皓燐／魔神龍／楊泯貽），而 gamelog 早已記錄新名——是既有的
  `_sync_player_names` 自 2026-07-04 每日排程停用後沒人跑，屬**維運缺口非程式 bug**。已手動同步 27 人。
- Log：
  - 07-15 ruan6047 提供資料源並指出「早期只有總教練與教練兩級」；Opus 盤點 8 區塊 vs 現有資料，定範圍：做教練團／異動／二軍獎項，跳過次級來源的逐年陣容
  - 07-15 執行完成（Opus-4.8@Claude Code）：
    - 182/246 頁解析（64 頁為總覽頁與「業餘N年」＝中職成立前，正確略過）；解析殘渣 0。
    - **教練團 1,010 筆／284 人（1990–2025），其中 92 人先前完全沒有**（中村典夫／成田幸洋／石橋文雄
      等早期日籍教練）。職稱**照抄原文**，分佈驗證只有總教練／教練／領隊／副領隊四種，無自創職稱。
      兩種頁面格式並存（早期兩級無分層；近年「一軍/二軍教練團」職稱標在括號內），皆支援。
    - 異動 1,397 筆；獎項 101 筆（**二軍 18 筆，先前 0**）；改名 13 筆（隊史 7＋官方 6）。
    - **閉環**：新發現的 92 位教練回灌 `person_history` 種子（`--scope staff`），82 人補上生涯歷程——
      否則資料是斷的（查得到他 1995 在味全當教練，卻查不到生平）。
  - 07-15 **第三方交叉驗證** `championship_managers`：年度戰績表的總教練欄獨立於維基球隊條目與
    twbsball 個人條目。有資料的 **15 個冠軍年 15/15 相符、零不一致**；2007 統一該頁列出呂文生／羅國璋／
    大橋穰三人，canonical 選的呂文生確在其中——三來源同指一答案。
  - 07-15 過程中的自我回歸（測試攔下）：為修教練名單的「季中換人」箭頭（黃煚隆→林瑋恩＝兩人）而在
    切分函式加箭頭切分，卻把改名欄的「舊名-->新名」（同一人，語意相反）也切開，改名解析 7→0 筆。
    已改為 `split_arrow` 參數區隔，兩種語意皆有測試釘死。
