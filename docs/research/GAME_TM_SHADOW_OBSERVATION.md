...
| 7 | 2026-07-27 | 3 | 9 | 9/9 | 0/0/0/0 | 0 | 觀測窗第 3 天；當日窗口內全數已完賽，無未開打場 |
| 8 | 2026-07-28 | 3 | 8 | 6/6 | 0/0/2/0 | 0 | 觀測窗第 3 天 |
| 9 | 2026-07-29 | 3 | 8 | 5/5 | 0/0/3/0 | 275 | game_sno 227 剛完賽，273 筆 `only_shadow_pk` + 226/227 各 1 筆 `local_lag`——同 run_id=5 的正式表時間差模式，非缺陷，預期下次觀測即消失 |
| 10 | 2026-07-30 | 3 | 6 | 5/5 | 0/0/1/0 | 642 | game_sno 229/230 剛完賽（639 筆 `only_shadow_pk`）+ 228/229/230 各 1 筆 `local_lag`；第三度復現同一良性時間差模式（見 run_id=5、9），無 `cell_mismatch` |
| 11 | 2026-07-31 | 3 | 9 | 6/6 | 0/0/3/0 | 333 | 僅 game_sno 231 剛完賽（332 筆 `only_shadow_pk`+1 筆 `local_lag`）；226-230 皆已 0 差異，第四度印證同一良性時間差模式會在 1-2 天內自然消失 |
| 12 | 2026-08-01 | 3 | 10 | 7/7 | 0/0/3/0 | 0 | 觀測窗第 8 天；`requests_schedule=2`——首次實測跨月邊界（window 橫跨 7/8 月），schedule API 正確合併兩個月份回應且 dedupe 無誤，0 差異 |
| 13 | 2026-08-02 | 3 | 10 | 7/7 | 0/0/3/0 | 559 | game_sno 235/236 剛完賽未追平（302+257 筆 `only_shadow_pk`，231/232/234 皆已 0 差異）；同一良性時間差模式，這次連續兩場未追平（推測跨週末/連假批次處理較慢），仍無 `cell_mismatch`。另注意 233/237（同為大巨蛋）雙邊皆 0 列，shadow/prod 一致非差異，屬資料源本身缺口非本卡範圍 |
| 14 | 2026-08-03 | 3 | 9 | 9/9 | 0/0/0/0 | 337 | 235/236 已追平（0 差異，驗證 run_id=13 lag 如期消失）；game_sno 240 剛完賽（336 筆 `only_shadow_pk`，同一良性模式）＋**1 筆 `cell_mismatch`**：game_sno 235 pitcher_acnt 0000007293 pitch 3 的 `content` 文字正式表為「趁傳進壘」、shadow（單場 API 即時抓）為「因右外野手 傳捕手失誤上二壘」——與已 spawn 的 follow-up task（官方賽後修正文字未回補正式表）**同一類已知問題的第 2 次復現**，非新缺陷、非 Gate 3 範圍。另 237/238（同為大巨蛋）再度雙邊 0 列一致，累計 3 場同場館 0 列，疑似大巨蛋 TrackMan 覆蓋缺口未列入既有場館覆蓋清單，已詢問需求方是否另開追蹤卡 |
