...
- 非目標：
  ...
  - ~~WPA 排序~~ **2026-08-06 需求方修訂**：禁令精確化為「WP 機率**數字**不得作為
    敘事展示核心」；**|ΔWP| 作關鍵打席選取訊號＋直接顯示 ΔWP 擺動值＝可**
    （同日二次升版：需求方實看後裁定顯示變化量更直覺）——統計依據：VAL1 實證
    辨別力真實（Brier 0.153 vs 基準 0.245）；變化量比水平值穩（兩端點同帶偏差
    相消，中段 ±4–6pt 水平偏差對 Δ 影響較小）；且 WP 曲線本已公開展示、擺動
    視覺可讀，per-play 數值未增加新的誠實暴露面。守門：badge tooltip 帶偏差
    揭露、methodology 註明依據與 caveat、機率水平值仍不作宣稱。屬統計紅線
    修訂，跨家族查核重點審。
