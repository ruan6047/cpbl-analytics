...
- as-of：**2026-08-07**　iteration：**2**（R1／R2 兩輪跨家族查核 REQUEST_CHANGES 後修正；
  **主結論「不可用」兩輪皆未被推翻**。R1 修 §3.1 的證據純度與 §1 的可重現性，
  R2 修 §3.1 正文對該數字的**稱呼**——見各節 iteration 標記）
...
**主證：105 筆投手列中 16 筆不一致，即通過兩道閘門後的 run-attribution mismatch rate
