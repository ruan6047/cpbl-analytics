...
|---|---|---|---|---|
| OPS-STATE-PLANE-MIG1 | 任務狀態面遷移至 GitHub Issues/Projects | 🏁完成 | —不適用 | [`tasks/OPS-STATE-PLANE-MIG1.md`](tasks/OPS-STATE-PLANE-MIG1.md)（T3🟡流程；WF-22 Wave 1 主卡；merge `dbbad73`；Sonnet 新 context 查核 APPROVE（41/41 對帳獨立重現、B2 契約校讀通過、1 項 merge 時必辦的 cutover 文字對齊已由祕書 `97c2cf5` 落款）。三段交付：Task 1 欄位表達力實測（13 欄 12 全表達、兩缺口經需求方裁決凍結：最後交接=TEXT 完整 ISO、資源宣告=body fenced JSON）→ Task 2 40 卡遷移＋Project #4 建置 → Task 3 七凍結卡 Issue 處置＋resync 41/41＋契約雙軌改版。**cutover 於 2026-08-04T23:47 由需求方宣告（終筆 `8271d7c`），events.jsonl/TASKS.md 投影自此封存**；本卡 release 是新狀態面上第一個完整 lifecycle（wfcli handoff）。執行亮點：自抓 dispose 誤發留言 bug、母體漂移如實記錄、trailer 授權重建樹逐位相同）|
| DEV-TRAILER-GUARD-PR-CHECKOUT1 | trailer 守衛在 pull_request 觸發的 CI checkout 上恆定假陽性 | 🏁完成 | —不適用 | [`tasks/DEV-TRAILER-GUARD-PR-CHECKOUT1.md`](tasks/DEV-TRAILER-GUARD-PR-CHECKOUT1.md)（T2⚪一般；兩 iteration、一輪 REQUEST_CHANGES＋一輪 APPROVE；PR #42 merge。iteration 0 的教訓最有價值：判別器 gate 4 假設 synthetic merge 訊息 base 側是分支名，10 個本機測試全綠，卻被驗收條件 4 的真實 pull_request CI run 揭穿——**GitHub 實際格式是雙 SHA**（fetch 真實 commit 54373c4 證實），8 個 fixture 因共享同一未實查前提而對缺陷免疫；「前提逐條附實查證據」與「真實 run 證據不接受本機模擬」雙防線的實戰案例。iteration 1 改雙 SHA 結構比對＋gate5 可選 rev-parse 強化（可退化），查核者以真實 GitHub 產物直接驗證＋15 項對抗檢查閉環。附帶發現 CI pytest 結構性慢（763s vs 本機 30s），依需求方裁定 cutover 後開調查卡）|
| INGEST-PLAYER-BIO-GAP2 | 補齊 14 人的 handedness 與 batch 2 其餘 bio 欄 | 🏁完成 | ✅已驗證 | [`tasks/INGEST-PLAYER-BIO-GAP2.md`](tasks/INGEST-PLAYER-BIO-GAP2.md)（T2🔴資料正確性；merge `8f6fe7c`；Sonnet 新 context 查核 APPROVE 零阻塞（2 info）：14 人 bats/throws 全表 NULL 14→0 SQL 實查、country/birthday 與 GAP1 逐位相同、COALESCE 只補缺、範圍恰 14 筆、快取重用裁定可接受（byte 級核實 0 次站台請求）；8/4 生產同步＋prod 重建對帳逐位相同後 release）|
| INGEST-SPLITS-IMPORT-RESTATE1 | 重建 2025 本土／外籍分項以吸收 bio 補值 | 🏁完成 | ✅已驗證 | [`tasks/INGEST-SPLITS-IMPORT-RESTATE1.md`](tasks/INGEST-SPLITS-IMPORT-RESTATE1.md)（T3🔴資料正確性；merge `bb7300f`；GPT-5.6@Codex 跨家族 APPROVE 零阻塞：671 列變動（新增18＋值變653）全在 batting_splits@2025 家族3、獨立重現差集 0、VS.外籍 +4537 本土 0、生涯 9999 與其餘三表零變動、canonical PA 261 組全對；執行者刻意直呼 build_splits 完全不碰 build_career（生涯零寫入有 updated_at 佐證）；**範圍外發現分流 `INGEST-SPLITS-IBB-GHOST1`**：幽靈島規則吞零投球手勢故四 26 席、RECALC1 全命中前提待重驗；前態唯一快照 artifacts/restate1-2026-08-04/ 勿刪；8/4 生產同步＋prod 同款重跑對帳逐位相同後 release。另留痕：PM 派工指令自相矛盾（merge vs 禁 merge）由執行者正確拒絕解決）|
| DEV-EVENT-REPAIR-ANCHOR1 | schema-repair 的歷史證據不可偽造 | 🛑已停止 | —不適用 | [`tasks/DEV-EVENT-REPAIR-ANCHOR1.md`](tasks/DEV-EVENT-REPAIR-ANCHOR1.md)（T3🟡流程；2026-08-04 工作流檢討決議 10 批准封存：狀態面遷 GitHub Issues 後 events.jsonl 退役，反偽造標的消失；rev2-4 的 anchor commit 設計思路留檔供未來 append-only store 參考）|
| OPS-CONTROL-PLANE-PR-GUARD1 | 將 control-plane 轉為受保護 PR 的機械守衛 | 🛑已停止 | —不適用 | [`tasks/OPS-CONTROL-PLANE-PR-GUARD1.md`](tasks/OPS-CONTROL-PLANE-PR-GUARD1.md)（T3🟡流程；決議 10 封存原 scope：control-plane 不再走 git；程式碼面 main 保護重切 `OPS-CODE-BRANCH-PROTECT1` 承接；其 Discovery 的 CI 基線工作已先後催生 `DEV-CI-SCORELESS-DB-SKIP1`（🏁）與 `DEV-TRAILER-GUARD-PR-CHECKOUT1`；worktree 已清、本地分支保留）|
| DEV-REVIEW-DEACCEPT-TRAIL1 | plain-review 翻案缺 correction 留痕（WF21-R-13） | 🛑已停止 | —不適用 | [`tasks/DEV-REVIEW-DEACCEPT-TRAIL1.md`](tasks/DEV-REVIEW-DEACCEPT-TRAIL1.md)（T3；決議 10：翻案留痕概念併入 Issues 審核契約移植——MIG-1 Task 3 契約改版收錄，事件載體退役）|
| DEV-REVIEW-PREFLIGHT-GATE1 | 前置查核關卡改為 preflight 的機器可讀宣告 | 🛑已停止 | —不適用 | [`tasks/DEV-REVIEW-PREFLIGHT-GATE1.md`](tasks/DEV-REVIEW-PREFLIGHT-GATE1.md)（T3；決議 10：preflight 宣告改由祕書 CLI assign／handoff 前置檢查承接（WF-22-CLI1））|
| DEV-REVIEW-PREFLIGHT-SELFCHECK1 | 派審前自檢交接前提 | 🛑已停止 | —不適用 | [`tasks/DEV-REVIEW-PREFLIGHT-SELFCHECK1.md`](tasks/DEV-REVIEW-PREFLIGHT-SELFCHECK1.md)（T2；決議 10：併入祕書 CLI 派審自檢（WF-22-CLI1））|
| DEV-CI-RED-OWNERSHIP1 | main 紅燈無歸屬：「非本卡引入」沒有強制去向 | 🛑已停止 | —不適用 | [`tasks/DEV-CI-RED-OWNERSHIP1.md`](tasks/DEV-CI-RED-OWNERSHIP1.md)（T3；決議 10：紅燈歸屬改為祕書 doctor 職責（WF-22-CLI1 doctor））|
| DOC-CARD-SPEC-RULES1 | 卡面規格三條規則（輸入空間／性質而非示範／同病自檢） | 🛑已停止 | —不適用 | [`tasks/DOC-CARD-SPEC-RULES1.md`](tasks/DOC-CARD-SPEC-RULES1.md)（T3；決議 10：三規則需求已記入 ai-workflow `WF-22` 父卡里程碑，Wave 2 canonical 卡面範本改版吸收）|
| DEV-CI-SCORELESS-DB-SKIP1 | CI api job 因單一測試未循同檔案模式 skip DB 不可用而長紅 | 🏁完成 | —不適用 | [`tasks/DEV-CI-SCORELESS-DB-SKIP1.md`](tasks/DEV-CI-SCORELESS-DB-SKIP1.md)（T2⚪一般；一個 iteration、獨立查核 subagent APPROVE 零阻塞 finding（2 筆 info）；merge PR #41 `7b611f4`，執行者依需求方 2026-08-04 授權代行。**發現於 `OPS-CONTROL-PLANE-PR-GUARD1` Task 1 取得 CI 綠色基線時**：main 連續三次 run 的 `api` job 皆因 `test_scoreless_streak_api.py::test_every_counted_appearance_is_officially_er_zero` 直接呼叫 `load_appearances()`、繞過同檔案其餘測試共用的 `_get()` skip-on-no-DB 防護，在 CI（無 Postgres）拋出未捕捉的 `PoolTimeout`。修法比照 `_get()` 補一層 `try/except pytest.skip`，斷言邏輯不變；本機無 DB 模擬（`DATABASE_URL` 指向無人監聽埠）與獨立查核的隔離 worktree 重跑皆證實：修正前 target 測試 1 failed，修正後 0 failed（乾淨 skip）。**驗證方式本身也是一個發現**：本機 push-to-branch 的 CI run 不會觸發（`ci.yml` 只在 `push: [main]` 與 `pull_request` 上跑），必須真的開 PR 才能得到 required-check 會用的實際訊號——這也是本卡執行者第一次為此專案實際走 GitHub PR merge（而非既有慣例的本機 `git merge --no-ff` 直接 push main）。**副產物比本卡目標更重要**：真實 PR 觸發的 CI 意外曝露了 `test_commit_trailers.py` 的結構性缺陷——GitHub `pull_request` checkout 產生的 synthetic merge commit 不帶任何 trailer，且守衛判定範圍必然把它算進「新 commit」，導致**任何** PR 的 `api` job 恆定失敗、與 PR 內容無關；已另立 `DEV-TRAILER-GUARD-PR-CHECKOUT1`（📥Backlog，需求方裁示暫不執行），並回頭更新 `OPS-CONTROL-PLANE-PR-GUARD1` 的阻塞鏈。查核 finding 兩筆皆 info／non-blocking：卡面缺 `review_independence` 欄位（以卡面〈查核〉欄原文替代已足夠）；卡面 AC4「同 SHA api／web 皆 success」形式未達成但根因結構性不同且發生於本卡未觸碰的檔案，兩者皆不阻塞 merge。**流程缺口如實記錄**：執行期間漏建本機 `lease.json` telemetry，純本機層級、不影響 lifecycle 正確性）|
| DEV-REVIEW-PROMPT-BASE1 | 查核提示詞在交付 SHA 的 worktree 內假性失敗 | 🏁完成 | —不適用 | [`tasks/DEV-REVIEW-PROMPT-BASE1.md`](tasks/DEV-REVIEW-PROMPT-BASE1.md)（T2🟡流程；一個 iteration、獨立查核 APPROVE 零 finding；merge `06dba9f`，執行者依需求方 2026-08-04 授權代行。修的是**訊息指控錯對象**：`review_prompt.py` 從自身 checkout 讀 event log，而 control-plane 依契約只存在於 main、執行分支不攜帶，於是在交付 SHA 的 detached worktree 內產生提示詞必然讀不到 handoff，舊訊息卻一律報「尚未交付查核」——而 `HANDOFF_CONTRACT.md` §3 正要求查核者先建那個 worktree，**照清單順序做的人第一步就會撞到**。2026-08-04 `DEV-BASELINE-GUARD-DECL1` 因此被誤退一輪，事件其實三筆齊全。**三件值得後續參照的事**：(1) **選了「診斷」而非「改讀 MAIN_ROOT」**——靜默換來源會讓「這份提示詞根據的是哪一版 event log」不可追，比假性失敗更難查；診斷用的兩個來源（主 checkout 檔案、`origin/main` blob）不參與提示詞內容，只分辨成因，故卡面驗收條件第 3 項判 N/A；(2) **兩種成因的輸出刻意無任何共同行**（測試 `test_the_two_missing_handoff_causes_do_not_share_a_message` 守住）——查核者要能自行判斷該修什麼，而不是回頭問執行者；(3) **查核者是按自己的流程實際踩到情境 (b)**，非重放執行者腳本，缺陷與修復在查核端獨立重現了一次。**已知限制（執行者主動揭露、查核未開 finding）**：診斷只在該 checkout 的腳本含本次修正時才出現，本次合併前分出的分支跑起來仍是舊訊息——這是「腳本從被查的那棵樹跑」的本質限制，緩解是 §3 的先後註記，故那一行不是附帶品而是本卡的一半）|
| DEV-BASELINE-GUARD-DECL1 | spec 基線守衛只認宣告值，說明文字不再放行 | 🏁完成 | —不適用 | [`tasks/DEV-BASELINE-GUARD-DECL1.md`](tasks/DEV-BASELINE-GUARD-DECL1.md)（T2🟡流程；兩個 iteration、一輪 REQUEST_CHANGES＋一輪跨家族 APPROVE；merge `3636066`，執行者依需求方 2026-08-04 授權代行。修的是**同型缺陷的第三次**：守衛先只擋哨兵值「—」（填卡名照過），改抽版本 token 後又從**整段欄位含說明文字**抽取，於是 `spec 基線：v2（v1 範圍過窄…）` 因說明提到父卡版本而放行——`INGEST-PLAYER-BIO-GAP2` 與 `INGEST-SPLITS-IMPORT-RESTATE1` 兩卡同時中招、全綠送審，由獨立查核者 PREFLIGHT_FAILED 擋下。**三件值得後續參照的事**：(1) **取樣範圍本身就是性質的一部分**——「檢查該成立的性質」不足以保證正確，還要問「從哪一段文字檢查」；(2) **同一條規則兩份實作必然分叉**：`review_prompt.baseline_check()` 另寫一份抽取，方向相反地誤報，對全庫 65 張有父卡的卡面實測 **30 張假警報**（凡版本後還有任何文字一律中招），並行的 `00b9ab4` 還為此把卡面註記整個拿掉當變通；收斂為單一抽取器後假警報全消、真不一致的 4 張不變、由通過翻成不通過 0 張；(3) **查核 finding `DECL1-F001` 打中的是執行者沒想到的輸入空間**——版本 token 無字元邊界，`spec 基線：rev1` 被抽成 `{v1}` 放行，而卡面正好有「卡面修訂：rev2」這種相鄰欄位，兩欄混用正是本卡要擋的錯誤本身。修法刻意讓**失敗方向偏嚴**（抽不到版本即硬紅），過嚴只逼人把欄位寫清楚、過寬會讓錯的基線交付。非阻塞 finding `DECL1-F002` 亦成立：交付摘要宣稱的 32 張是 rebase 前快照，正確值 30，差額恰為 `00b9ab4` 已變通的那兩張——**掃描數字必須綁定交付 SHA**。後續卡 `DEV-REVIEW-PROMPT-BASE1`（📥Backlog）：提示詞產生器在交付 SHA 的 detached worktree 內會假性回報「沒有 handoff event」，本卡查核期間實際造成一輪誤退）|
| INGEST-PLAYER-BIO-GAP1 | 補齊 14 位球員的 country／birthday | 🏁完成 | ✅已驗證 | [`tasks/INGEST-PLAYER-BIO-GAP1.md`](tasks/INGEST-PLAYER-BIO-GAP1.md)（T3🔴統計；merge `9393646`；跨家族查核 GPT-5.6@Codex APPROVE 零 finding；8/3 手動同步生產並實查結案。**缺口成因是兩個來源的交集，不是覆蓋缺口**：opendata 涵蓋止於 2024，而 bio 爬蟲最後一次走訪這 14 人早於 `cf9d8b8`（第一版會讀國籍／生日的 `parse_bio`）。`classify()` 對 `country IS NULL` 保守回 `local`，使 2025 有 164 個先發席次（22.8%）被標成本土，且該批是整季輪值洋投（144.0／123.0／120.2 局），對 `starter_import_diff` 是雙向污染而非樣本流失。**最有價值的產出是那個不利結果**：補值後照凍結協定重跑敏感度，**2026 方向警訊未消解**（Δ +0.006116 → +0.005751），且 **2025 效果反而變弱**（−0.008404 → −0.007769，99% CI 上界由 −0.000806 變 +0.000536，該季由「CI 排除 0」變「含 0」）、池化邊際變薄——**推翻了診斷輪的機制推論**（「把最強的洋投移回洋將側，訊噪比應提升」）。證據不足以分辨「機制假設在 2025 不成立」與 「n_val=359 噪音」，故未對成因做任何宣稱；紅線 2 保住的正是這個。**三件值得後續參照的事**：(1) `players.country` **不是恆為非 NULL**，且 `bio_updated_at` 非 NULL 只證明「被走訪過」、不證明「被讀得懂該欄的解析器版本走訪過」——判斷 bio 欄可信度須與 `cf9d8b8` 比對時間戳，已寫進 `imports.py` docstring 當維護註記；(2) 一輪 `PREFLIGHT_FAILED` 的根因是執行者**在 commit 之前跑 pytest**，`test_commit_trailers.py` 取樣範圍為空而 `skip`——**同型錯誤 2026-07-29 已發生並已寫入記憶，仍復發**，且 repo 既有 commit 訊息慣例早已警示；偵測訊號為 **skipped 數 ≠ 3**；(3) 診斷輪與補值輪相隔六天（Gate 3 觀測窗），中間的範圍閘門從對稱改為非對稱才容得下「斷路器中止後續跑」與「補完後冪等重跑」——**對稱的『集合必須完全相同』會打死這兩個合法情境**。**明列未解**：2025 補值後為何變弱無答案，留給未來融合升級卡；`review_independence` 宣告 `[context]` 但實際達成跨家族，差異由 CORRECTION-018 並列記錄、未回頭改宣告值。後續卡：`INGEST-SPLITS-IMPORT-RESTATE1`、`INGEST-PLAYER-BIO-GAP2`，皆 📥Backlog） |
| DEV-EVENT-SCHEMA-GUARD1 | event log 寫入端無 schema 把關 | 🏁完成 | —不適用 | [`tasks/DEV-EVENT-SCHEMA-GUARD1.md`](tasks/DEV-EVENT-SCHEMA-GUARD1.md)（T3🟡流程；四個 iteration、四輪跨家族查核（Gemini 3.6 Flash），三輪 REQUEST_CHANGES 觸發**本專案首例 WF-21 升級檢查點**，需求方核可 epoch 0→1（`reason=replan`、執行者不變）；merge `61b5092`。修的是「schema 驗證只在讀取端執行、寫入端完全不設防」——壞資料可順利進檔案、被 commit、被 push，然後**永久卡住 ledger**（`--write`／`--check` 每次都在同一行拋 `ValueError` 而中止，而 `TASKS.md` 是本專案宣告的當前狀態事實來源），且契約要求 append-only 又不提供 malformed 的合法修復程序，兩者互鎖。**三件值得後續參照的事**：(1) 查核者連續四輪提出的 finding **全部落在執行者於派工提示詞中自陳「判斷不了」之處**——該自陳具實質診斷價值而非形式謙辭，日後派工可延續此做法；(2) 執行者反覆犯同一類錯（在 commit 之前跑驗證因而測試看不到自己的 commit、設計守衛卻留單點繞過、發明事件型別卻沒實際走過、發現漏洞卻只把合成情境測得更完整），**四次都由外部關卡擋下、非自我發現**；(3) 執行者曾誤用 `review-correction` 想把前輪 finding 標為 `resolved` 以清升級計數，實質是粉飾三輪歷史，被契約「resolved 未撤銷不得洗掉真實歷史」擋下。第四輪的 F005 依 `replan` 承諾拆出 `DEV-EVENT-REPAIR-ANCHOR1`。**結案五步當時只做了第一步**（close 事件寫了、Ledger 移出了，但卡檔未封存、索引未加、分支未清），由 8/3 的看板巡檢補齊——與本卡主題同型：留痕動作沒有任何一步是下一個 session 一定會碰到的） |
| INGEST-SPLITS-RECALC1 | 修正分項重算的代打誤切重複計數並重建 | 🏁完成 | ✅已驗證 | [`tasks/INGEST-SPLITS-RECALC1.md`](tasks/INGEST-SPLITS-RECALC1.md)（T4🔴資料正確性；merge `46aed5e`；Phase P 生產重建 2026-07-31 完成、submodule `ef29798`／Deploy `30615339612` success，8/3 結案。修的是 `splits_calc` 與 PA-FIX1 同型的島切分缺陷——**打席中途代打被切成兩個 PA 而重複計數**，曝險 83 筆／82 場（2018–2026）並連帶造成打序位移污染。修法為 canonical 合併邏輯（`splits_pa_merge.py`）併入 `calc_t2`，構成與 canonical `pa_build` **同判準不同實作路徑的殊途同歸驗證**。重建對帳：預期 delta **17,997 格全數命中、非預期變動 0**、`*_vs_team` 逐格未變、**冪等 diff=0**。**生產寫入的前置閘門是另一張卡**：Phase P 等 `OPS-BACKUP-EMPTY1` 交出可驗證的備份才放行（`pre-RECALC1` 備份 240MB／`CREATE TABLE` 99，**勿刪**）。**衍生調查留下的結構性風險（封存但未解）**：`sync_table` 是純 UPSERT 無 DELETE，本機刪除的列在生產永久留存，**且只比對值的對帳抓不到**（本次靠比列數才發現，splits 112 列＋`model_versions` 1 列已清）；44 表計數普查未見普遍化跡象，但該方法**不排除殘列與未同步互相抵銷**，故只能說「無普遍化跡象」。未來任何「本機 DELETE+INSERT 重建」的表都會複發，須以生產端重建而非依賴同步。**兩筆 open finding**：TRAILER-001（blocking）結案時實測已修（trailer 測試 2 passed）；REVIEW-EVIDENCE-002 為記錄型（查核者數字不可重現，留存供後續派工參照），保持 open 是正確狀態） |
| OPS-BACKUP-EMPTY1 | 生產每日備份長期產出空檔 | 🏁完成 | ✅已驗證 | [`tasks/OPS-BACKUP-EMPTY1.md`](tasks/OPS-BACKUP-EMPTY1.md)（T3；需求方人工查核 APPROVE 零 finding；merge `9908e19`；8/3 結案。**生產災難復原能力曾歸零 88 天**：VPS cron 的 86 份備份全是 20-byte 空檔。**兩個教訓勝過交付本身**——(1) **失敗會留下看起來像成功的產物**：那 86 份**每一份都通過 `gzip -t`**，因為空 dump 本身是合法的 gzip 串流；檢查「檔案有沒有壞」不等於檢查「有沒有內容」，又一次「檢查看得見的標記而非該成立的性質」。(2) **能產出 ≠ 還原得回去**：只有還原演練看得到——以修復後備份還原至拋棄式 PG17 容器得 `FK=15`／`livelog=1337619`／`tables=99`，而 **`FK=15` 是決定性證據（修復前為 13）**，`ON_ERROR_STOP=1` 會在外鍵階段中止（`convalidated=t` 卻實際不成立的 FK）。查核者以**變異檢驗**證明守衛真的會擋：連線失敗 `exit=23`、**dump 空但 exit 0 → `exit=66` 並報「CREATE TABLE 0 < 門檻 90」**，正是 88 天缺陷的失敗模式。現行 = 開發機 `backup-prod-db.sh` 備整庫 + 內容門檻，`refresh-cpbl-prod.sh:116` 已接上。**明列未處理項**：off-site 單點與還原演練未自動化兩項至今未做、無後續卡，勿把「備份已修好」誤讀為災難復原已完備） |
| UX-TOKEN-ACCENT-CONTRAST1 | accent 在 surface-2 底上未達 WCAG AA | 🏁完成 | ✅已驗證 | [`tasks/UX-TOKEN-ACCENT-CONTRAST1.md`](tasks/UX-TOKEN-ACCENT-CONTRAST1.md)（T2；iteration 2 APPROVE；merge `a739b60`；主站 `453a418`／submodule `3127ad0` 上線，生產 CSS 實測 `--color-accent:#d12638`（深色 `#ff5a6a`）、舊值 `#d62839` 已不存在，8/3 結案。**真正的缺陷型態是「同一個顏色有兩份真相」**：CSS canonical 值改了，但 JS 側的 SSR fallback 鏡像沒跟上，而既有 drift 守衛不涵蓋語意色——blocking F1 補的正是這個守衛缺口（新增由 `globals.css` canonical 值驅動的 fallback 對照）。**三筆 finding 於 8/3 結案時逐筆實測複驗皆已修**：`chart-theme.ts:45-46` 已為 `#d12638`（同檔 125／129 的 `#d62839` 屬 `hr`／`whiff` 類別色，不在範圍）、`docs/design/` 全樹 grep 舊值零命中且對比表已含 `on surface-2` 欄、卡面已有〈Discovery 書面答案〉節。**留痕缺口誠實記錄**：三筆在 event log 的 status 仍為 `open`（iteration 2 APPROVE 未帶 `finding_updates`），結案只記錄實測結果、不代寫他人查核判定） |
| UX-BRAND-HOME1 | 站名品牌化與首頁門面升級 | 🏁完成 | ✅已驗證 | [`tasks/UX-BRAND-HOME1.md`](tasks/UX-BRAND-HOME1.md)（T3；三輪查核，iteration 3 APPROVE；merge `44deedf`（29 檔 +986/−788）；主站 `453a418`／submodule `3127ad0` 上線，生產首頁 `<title>` 已為「Ruan's 中職數據實驗室｜中華職棒數據視覺化」，8/3 結案。**merge 後以測試數字對照證明交付真的進了 main**（`npm test` 253→255、`build:check` static pages 11-11→14/14）——這個對照正是第二輪查核審錯樹時沒做的檢查。**`FINDING-REVIEW-WRONG-TREE-003` 是記錄型 finding，保持 `open` 是正確狀態**：第二輪跨家族查核審錯樹且證據不可重現、該輪未提出任何實質意見，disposition 明訂「不阻擋交付，留存記錄供後續派工選擇查核者時參照」，其後確實被引用（子卡 FIX1 走 human 分支的裁定理由即引述本筆）。**結案時查證的兩項需求方手動後續**：主站 admin `projects.title` 改名無法由公開端點確認（保留待辦，且 `slug` 不得更動否則專案頁自造 404）；GitHub repo description 實測仍為空，未完成。〈SEO 風險〉的 description 補償缺口由 `UX-BRAND-HOME1-FIX1` 補齊） |
| UX-BRAND-HOME1-FIX1 | description metadata 未隨品牌與副標收斂 | 🏁完成 | ✅已驗證 | [`tasks/UX-BRAND-HOME1-FIX1.md`](tasks/UX-BRAND-HOME1-FIX1.md)（T2；需求方本地人工審 APPROVE 零 finding；merge `f93b214`；主站 `88dc588`／CI `30783459681`→Deploy `30783575497` 上線並於生產逐字複驗，8/3 結案。父卡〈SEO 風險〉要求 title 與 description 兩處補償、當時只做了 title，description 因而與新品牌脫節。**取捨線是本卡重點**：同句後段「資料缺口會揭露」與次段「資料來自 cpbl-opendata…」刻意未動——缺的是資料不是數據，未把「資料」當錯字批次替換（卡面紅線 2）；`og:description` 由 `description` 衍生而不另設欄位，不製造第二個真相來源。人工審輪中需求方主動再縮短首頁 description（59→53 字，行動版 SERP 截斷點）。**走 human 分支而非另派跨家族查核是明示的流程取捨並留痕**：T2、改動三行文案、驗收全可由指令重現，且同批工作已觀察到跨家族查核者本身會出錯（父卡 `FINDING-REVIEW-WRONG-TREE-003`）——小卡上增加關卡不必然增加正確性） |
| UX-LIVE-TRACKMAN1 | 賽中逐球 TrackMan 顯示 | 🏁完成 | ✅已驗證 | [`tasks/UX-LIVE-TRACKMAN1.md`](tasks/UX-LIVE-TRACKMAN1.md)（T3；iteration 2 跨家族 APPROVE 零 blocking；merge `8a52ea6`；主站 `b8b5df1`／Deploy `30738480435`，需求方於 live 場次實測逐球後 8/3 結案。**驗收條件自帶現場時機**：部署當下 `2026-A-238` 雖為 `phase=live` 但 `tracking_count=0`，需求方明訂必須等到一場 live TrackMan>0 才算完成、**不得以 final 回填代替**——這條「不得用較易取得的證據替代」的紀律是本卡的關鍵留痕。產品面守住同一條線：`tracking_count=0` 時只播報比分狀態，不畫空好球帶、不吐「未設置 TrackMan」假結論（與 `LIVE-SNAPSHOT-FIELDS1` 同源的設備斷言紅線）。**兩筆 non-blocking finding 的去向**：N1（每日 refresh 未跑 `cpbl-build-pa`，賽後逐球因而 fail-closed）轉由既有卡 `INGEST-PA-DAILY1` 承接；N2（`test_live_game_api.py` 的 PA query 守衛仍是原始碼字串比對而非行為驗證——本專案反覆出現的「檢查標記而非性質」同型病）**結案時明列為無承接卡**，屬需求方裁量） |
| UX-LIVE-GAME1-FIX1 | 未開賽不得顯示局數 | 🏁完成 | ✅已驗證 | [`tasks/UX-LIVE-GAME1-FIX1.md`](tasks/UX-LIVE-GAME1-FIX1.md)（T2；跨家族查核 Gemini 3.6 Flash APPROVE 零 finding；merge `c4c688f`；碼隨 `3127ad0` 那次 bump 上線，需求方生產實測後 8/3 結案。父卡部署後於生產發現未開賽場次顯示「▲ 1 局」——成因是 live worker 回 `inning=1` 佔位而前端直接採信。**修法不是特判該值，而是把「有沒有開打」變成一個明確的述詞**：新增 `hasStartedPlay` 與統一的 `inningLabel`，`scheduled`／`probable_announced`／`lineup_announced` 即使收到佔位值也回 null（顯示「等待賽況」且 aria-live 不帶局數），`live`／`final` 維持局數與上下半局，其餘 phase 以 `event_count` 認定（保留賽已打數局仍顯示、開賽前延賽隱藏）。**狀態列與 aria-live 改呼叫同一函式**，消除兩處各自判斷這個複發源） |
| UX-LIVE-GAME1 | 賽前情報到比賽中狀態板 | 🏁完成 | ✅已驗證 | [`tasks/UX-LIVE-GAME1.md`](tasks/UX-LIVE-GAME1.md)（T3；iteration 0 REJECT 四項 blocking → iteration 1 APPROVE 零阻塞；merge `e05e3dd`；主站 `a9861cb`／Deploy `30605694953` 上線，需求方賽事期間人工實測後 8/3 結案。**四項 blocking 是同一個病的四種長相——把 live 當成 final 來敘述**：沿用 `home_score + away_score > 0` 判 completed 會在賽中就吐出完封／逆轉／再見／MVP 等賽後結論（須以 canonical `phase=final` gate）；全場 H/E 總數被塞進第一局而製造出逐局煮粥語意（總數可顯示、逐局必須 unknown）；TrackMan 入口在 live 顯示「球場未設設備」結論；`/status` 回 200 但 `live_snapshot:null` 時清掉 last-known-good（須 fail closed 保留舊畫面並標中斷）。**驗收要求雙向驗證而非單向壓制**——查核者確認 live 時賽後結論全數消失、final 時全數恢復。輪詢實測 live 12.0s／final 0 req／hidden 50s 0 req 回前景立即恢復，外部 host 0 未觸及官網。**部署後於生產發現「未開賽顯示 ▲ 1 局」，未就地修改而另開 `UX-LIVE-GAME1-FIX1`**，去向明確） |
| LIVE-SNAPSHOT-FIELDS1 | canonical snapshot 保留官方既有欄位 | 🏁完成 | ✅已驗證 | [`tasks/LIVE-SNAPSHOT-FIELDS1.md`](tasks/LIVE-SNAPSHOT-FIELDS1.md)（T3；跨家族查核 iteration 1 REQUEST_CHANGES → iteration 2 APPROVE 零 finding；merge `bc62f91`；碼隨 `3127ad0` 那次 bump 上線、生產指標 `ae7e2ee` 複驗為祖先。**修的是三態語意被壓成兩態**：`skip_trackman` 的 `true`／`false`／`null` 原本共用同一句文案，把「資料尚未發布」誤述成「球場未配置設備」——一個關於**設備**的斷言，實際只有 `true` 能支撐（呼應 `SkipTrackman=false` 不可映射為 available 的既有紅線）。改為 `true` 才宣稱未配置、`false` 述資料尚未發布、`null` 用中性文案不推測原因，並把擊球落點／主審判決／好球帶三個入口統一走 `trackingEmptyMessage()`，消除同一事實三處各自措辭。**部署面與前兩次純前端不同**：同時更新 `prod_cpbl_live_worker` 與 `prod_cpbl_web`。**驗收條件自帶時間性**：新 snapshot 欄位只對新版 worker 抓過的場次生效、舊 snapshot 不回填，故真值驗證必須挑部署後才開打的場次，查核當下賽後 snapshot 已逾保存期而無法在瀏覽器重現，該條件改由真實 fixture＋解析測試＋執行期證據覆蓋。實測由需求方於 8/2 賽事後完成，8/3 補帳結案） |
| OPS-LIVE-SHADOW1 | VPS 隔離 live source observer | 🏁完成 | ✅已驗證 | [`tasks/OPS-LIVE-SHADOW1.md`](tasks/OPS-LIVE-SHADOW1.md)（T4🔴production 資安部署／資料證據；Antigravity Gemini 跨家族 APPROVE；merge `c0cc2290`；主站 `988a5cd`／Deploy `30207417582`。**暫時性觀測器的完整生命週期已走完**：7/26 部署 → 7/28 由 `OPS-LIVE-SHADOW1-FIX1` 修完賽場停輪詢 → 7/30 00:00 硬截止 clean exit → 7/30 唯讀對帳（manifest 4,614 entries、A-226／227／228 每場 HTTP error=0 且皆到 FINISHED、export manifest／checksum 生成）→ 證據消費卡 `LIVE-GAME-BACKEND1` 7/30 21:16 結案 → 主站 `3bee24c` 自 compose 移除 service 與 deploy verify 步驟、deploy run 恢復綠燈 → 8/3 結案。**刻意保留**：evidence volume `cpbl_live_shadow_evidence` 與 network `cpbl_live_shadow_egress` 仍在主站 compose，刪除 volume 屬不可回復操作須 ruan6047 明示；dormant 的 `live_shadow_observer.py` 保留（截止釘在碼內，啟動即於發 request 前 exit 0）。**驗證邊界**：結案未做 VPS SSH 實測，撤除依據為 protected main compose 內容與 deploy run 結果。**留下的教訓**：釘在 versioned code 的硬截止會讓部署管線在截止後**必紅**——deploy run `30468522739` 即因「observer still running after deadline」失敗並卡住兩張無關的卡，暫時性服務的撤除變更必須排在截止時點之前，不是之後） |
| INGEST-SPLITS-PA-SPLIT1 | 查證分項重算是否同樣重複計打席 | 🏁完成 | —不適用 | [`tasks/INGEST-SPLITS-PA-SPLIT1.md`](tasks/INGEST-SPLITS-PA-SPLIT1.md)（T4🔴資料正確性；**5 輪跨家族查核（Codex），三連退觸發 `🚨已升級` → 需求方裁定重規劃 spec v2**（首個以升級裁定重規劃收斂的卡），最終 APPROVE；merge `cc339f5`。**結論＝官方已發布分項有偏差且已量化**：`splits_calc` 與 FIX1 同型島切分缺陷重複計打席，曝險 **83 筆／82 場**（FIX1 的 296 誤切中 210 被幽靈島規則**意外擋對**、3 無結果）；H1 打序位移 82 個 (場次,球隊)／1,291 筆後續 PA（81×1＋1×2，污染家族 10）；選手層級**完整發布欄位** delta 2,435 rows／17,997 格／314 人（rate 欄 7,394 格），可直接作修正卡的預期 diff。方法：named-column 模擬器（18 pairs 對 calc_t2 逐格相等）＋組裝層保真（對 DB 已發布列 226,690／226,690 逐格全等）＋ box 逐場交叉驗證（非自比：legacy 88 筆不吻合 → corrected 剩 7）；跨投手合併島投球數逐列保留＋9.16(h) 責任歸屬。H2＝2018–2025 分項**從未對過帳**、當年快照**不可重現**（APART_COMBOS／updated_at 單一日期 2026-07-14／Phase 0 殘差無 artifact／備份範圍，證據 artifact 化＋守衛）；H3＝PA 隨 9.15(b) 歸屬 charged_hitter（2025/A/84 實證），官方偏離一例 2018/A/116（幅度 1 PA，修正卡需裁定口徑）。**方法論產出**：(1) 升級重規劃把已背書宣稱轉**回歸斷言**＋完成線**性質化**（新期望不推翻斷言者開新卡，不得單獨 REJECT），終結逐輪追加期望的迴圈；(2) 每日增量兩度打穿釘死數字 → **baseline 凍結窗**（game_date ≤ 2026-07-28）＋窗外自動歸因守衛，15 組守衛全數名稱識別；(3) REVIEW-010 Medium 無獨立指涉不可考的處置先例（correction 留痕＋不構成修復標的）。**後續（需求方保留）**：是否開分項重算修正卡；驗收對照可另開爬卡取官方 `/team/apart` 歷年值） |
| DEV-TRAILER-GUARD-SCOPE1 | trailer 守衛取樣範圍 | 🏁完成 | —不適用 | [`tasks/DEV-TRAILER-GUARD-SCOPE1.md`](tasks/DEV-TRAILER-GUARD-SCOPE1.md)（T2🟡工具；跨 session 獨立查核 iteration 1 APPROVE 零阻塞；merge `cb97be1a`。**修的是守衛自己犯了它要抓的病**——以 `origin/main..HEAD` 取樣，等於拿「不在 origin/main 裡」當作「這是執行分支的 commit」的標記，導致同一個 commit 的判定因推沒推過而翻轉（未推亮紅、push 後取樣範圍變空即轉綠——**一個會被 push 消音的守衛，等於鼓勵用 push 讓它閉嘴**）。改以**本地 `main`** 為邊界：`git push` 不移動任何本地分支，判定與 ref 同步狀態脫鉤；main 上的 Coordinator commit 因 `HEAD == main` 這個結構性事實**恆定排除**，而非因推沒推過。**執行者實測後排除了卡面給的兩個方向**：(a) 判「任何 remote-tracking ref」本質仍是 ref 存在性，不解紅線；(b) 反向擴大範圍納入 main，因歷史誤鳴率過高需先訂前綴／欄位政策，屬另一個決定。**查核者獨立重算誤鳴率**：`c3042f3..main` 226 commit 中 93 個（41.2%）缺至少一項；改用 canonical §6 對 T0/T1 的較寬判準（僅 `Requested-by`＋`Implemented-by`）為 91 個（40.3%）——**僅差 0.9pp，結論對判準不敏感**，執行者擔心的「Co-Authored-By 讓數字虛高」實測可忽略。**查核者自建 bare remote + work repo 重現 2026-07-28 實況**：舊版問題數 1→0（翻轉重現）、新版 0→0（消除），同時證明舊版確有該缺陷而非僅證明新版通過。ruff clean、pytest 820 passed/3 skipped、主測試 PASSED 而非 SKIPPED。**非阻塞 findings**：F1（Minor）本地 main 落後 `origin/main` 時會**反向**大量誤鳴（executor 寫 1 個 commit 卻被算進 4 個、報 3 個問題，而舊版在此情境反而正確）——因 `AI_RUNBOOK.md:267` 的 worktree 慣例不帶起點（從本地 main 開分支）而觸發不到，建議後續改為**邊界取本地 main 與 origin/main 中血緣較新者**，兩情境同時成立；F2 執行者把 93 vs 91 的落差歸因於「200 筆視窗滑動」有誤，實為兩人 REQUIRED 集合不同（與本卡主題同型的歸因錯誤）；F3 handoff payload 用 7 字元縮寫 SHA，違反 canonical §4.1 完整 40 字元要求。**本卡是 cpbl 第一個 `handoff-accepted` 事件**——WF-20 契約落地首例。**次要交付**：守衛盤點發現 `verify_deep_tm_backfill.py` 只印不驗，執行者正確地未自行擴張範圍而改為建議開卡（`DEV-VERIFY-TM-ASSERTS1`）） |
| GAME-RECAP-PA1-FIX1 | canonical PA 表的打席切分與出局數正確性 | 🏁完成 | ✅已驗證 | [`tasks/GAME-RECAP-PA1-FIX1.md`](tasks/GAME-RECAP-PA1-FIX1.md)（T4🔴資料正確性；**6 輪跨家族查核，iteration 3 觸發 `🚨已升級`（三連退）**，最終 APPROVE 零阻塞；merge `3e30dbf`；碼主站 `321c41f`、生產資料重建 2026-07-29 完成並驗收。**修好兩類缺陷**：134 個半局有 >3 個出局打席（builder 把單一打席切成多個——中途換代打時 livelog 的 `hitter_acnt` 變了但打席未結束）、1,174 組 `pre_state.outs` 不遞增（源頭 `game_livelog.out_cnt` 落後，改由 `content` 的「N人出局」推導）。合併判準需 `batting_order` 相同**且**有佐證（球數非遞減且已達 2 顆、或有 `更換代打` 公告），對 9 個真打席邊界實測不誤併——**「新打席首列球數必 ≤1」的直覺前提被自己實測推翻**（`2021/D/64`、`2018/A/4` 反例）。加 per-game fail-closed 不變式（半局出局打席 > 3 不發布），實測隔離 `2019/A/173` 且**單場重爬證實損壞在官網源頭**（重爬後 byte-identical）。依規則 9.15(b) 實作打席歸屬並新增 `end_hitter_acnt` 欄（migration 068）：代打者完成的三振記給被判第 2 好球者、其餘記代打者；全庫 296 筆跨打者、22 筆記原打者。**順帶修好 BUILD1 兩個潛在缺陷**：`game_pa_pitch_mappings` 唯一鍵含 `source_revision_id` 隱含「一個 tracking revision 只有一個 build」（migration 069 改 per-build）、`builder_upgrade` 守衛過嚴致 TrackMan 晚發布場次卡住。**方法論產出**：版本紀律——行為變更不 bump 版本會讓等價 noop 跳過副作用（iteration 5 因此被退，且執行者曾刪 build 湊數字，違反 append-only）；產生式產物必須改產生器而非手改輸出；「兩處用同一規則」須逐行核對迴圈。**結案時意外發現生產災難復原能力歸零**（86 份備份全空），另開 `OPS-BACKUP-EMPTY1`）
| UX-TEAM-HOTZONE1 | 近期表現熱區改用擊球品質口徑 | 🏁完成 | ✅已驗證 | [`tasks/UX-TEAM-HOTZONE1.md`](tasks/UX-TEAM-HOTZONE1.md)（T3；需求方多輪人工審核＋獨立查核 APPROVE 零 findings；merge `56ad6b3`；主站 `30e7ce4` 部署並於結案前複驗：舊 `/hot-batters` 404、`/hot-zone` 覆蓋缺口如實揭露、隊史紀錄野手5/投手4 零重複。熱區由近 7 天 OPS 改過程型（打者 14 天平均擊球初速 BIP≥5、投手揮空率 投球數≥20）＋新增投手熱區——**理由是指標選擇而非跟官方一致**：12 個打席的 OPS 幾乎純噪音。**命名紅線守住**：官方「出色擊球」判準未公開故不沿用官方名稱。同批交付需求方逐輪追加九項（小頁籤／垂直堆疊三欄／標籤精簡／同 stat 抑制 approaching／自我刷新不顯示原紀錄段／INFO 按鈕，但**樣本數與覆蓋揭露拒絕藏進 hover**）。**順帶修好全站缺陷**：Tooltip 的 cloneElement 以 undefined 覆蓋呼叫端 aria-label，部署前生產實測為 null、部署後為正確名稱——Coordinator 原診斷「呼叫端漏寫」是錯的。**待議**：筆電視窗（800–880）仍需捲動，縮進去需再砍約 136px（選項見 CLAIM-004）；官方排序鍵改採事宜暫緩，反解判準的探查結果已留卡面 Log）
| ML-PITCHER-SCORELESS2 | 提高連續無自責分尾段的採計率 | 🏁完成 | —不適用 | [`tasks/ML-PITCHER-SCORELESS2.md`](tasks/ML-PITCHER-SCORELESS2.md)（T3🔴統計；5 輪跨家族查核，最終 APPROVE 零阻塞；merge `f1aa1aa`；**結論＝無可行方向、以 memo 收攤**（卡面明訂為合格交付）。iteration 1 的 last_pitch 下界（採計率一度 24→29 場／126→156 outs）被**規則反例**推翻並撤回：總教練手勢故意四壞（9.14(d)＋5.05(b)(1) 原註）與投手犯規（6.02(a) 罰則）皆不需投球、且皆為自責分因素（9.16 及註2①）——**投手可在零投球下被記自責分**，故投球數耗盡證不出「其後無自責分」。**撤回外另有窮舉證明無中間地帶**：增益區包含於反例適用區（202,734 組中增益 53,988 全落在不安全區、正向反例 0），排除風險後增益為零，關掉「加個守衛再試一輪」；鏡像做法亦不成立（規則 5.10(g) 加註）。留下四個回歸測試含 AST 零邏輯變更斷言。**方法論產出**：對帳零例外只證一致性不驗前提真假；宣稱嵌手工數字必然漂移須改自動產生；守衛須在真正把關的環境可驗證其執行過（CI 淺層 clone 曾靜默 skip）。採計率維持基線 24/126，不變式是零例外而非絕對值。後續＝8/7 後查 stats 域逐局責任投手）
| OPS-LIVE-SHADOW1-FIX1 | 完賽場停止輪詢以保留 A-228 request budget | 🏁完成 | ✅已驗證 | [`tasks/OPS-LIVE-SHADOW1-FIX1.md`](tasks/OPS-LIVE-SHADOW1-FIX1.md)（T4🔴production request safety；Gemini iteration 2 APPROVE 零 findings；merge `c2fc864e`；主站 `29be1d54`／Deploy `30368723275`；production 自然 cycle 僅 schedule/A-228 成長，A-226/A-227 停止，evidence 1..3465 連續。父卡 7/30 對帳與撤除仍保留） |
| ML-PITCHER-SCORELESS1 | 投手連續無自責分局數（保守下界） | 🏁完成 | ✅已驗證 | [`tasks/ML-PITCHER-SCORELESS1.md`](tasks/ML-PITCHER-SCORELESS1.md)（T3；**11 輪跨家族查核**，獨立查核者最終 APPROVE 零阻塞；merge `aad35bb`（`--no-ff` 被他 session 的 `pull --rebase` 線性化，記線性尾端，`Reviewed-by` 因此只存於 Ledger）；主站 `4539a3c` 部署驗證：`/api/v1/records/earned-run-free-streak` 第 1 名呂彥青 28.1 局。**最終方法＝鴿籠下界**（`官方出局數 − 3 × 前綴局數`），完全不讀 livelog；前七輪以「半局存在／半局內列一致／整場總和／`pitch_cnt` 閉合／投手不得再入賽（規則 5.10(d) 證明該假設不存在）／`main_event_no` 密度」證明資料完整的路徑全被反例推翻，共同成因是**列的缺席不可偵測**。代價＝採計率一軍 24/343（7%），已誠實揭露，提升採計率另開 `ML-PITCHER-SCORELESS2`；窮舉對帳零例外；公開紀錄對照坎南 30.0（含 3.0 局來自尾段機制）、黃子鵬 33.2）
| UX-TEAM-FOCUS2 | 球隊頁「近日焦點」頁籤內容擴充 | 🏁完成 | ✅已驗證 | [`tasks/UX-TEAM-FOCUS2.md`](tasks/UX-TEAM-FOCUS2.md)（T3；需求方人工審核通過＋獨立查核 APPROVE；merge `420c3e5`；主站 `4539a3c` 部署驗證。交付下一場先發卡＋近期球員熱區；**熱區的近 7 天 OPS 口徑將由 `UX-TEAM-HOTZONE1` 取代**——12 個打席的 OPS 訊號太弱，改用過程型指標。派工時 Coordinator 曾以空白驗收條件派出、被執行者正確拒收，根因是只檢查卡面欄位未讀正文）
| UX-TEAM-RECORDS1 | 球隊頁「即將挑戰的紀錄」 | 🏁完成 | ✅已驗證 | [`tasks/UX-TEAM-RECORDS1.md`](tasks/UX-TEAM-RECORDS1.md)（T3；需求方三輪人工審核（句式列→資料卡→橫向網格）＋獨立查核 APPROVE 零 findings；merge `cfb0b12`；主站 `4539a3c` 部署驗證：台鋼雄鷹 9 筆「隊史新高」，後勁台鋼隊史投球局數 231＝79.1＋152.1（樂天三年正確排除），林泓育 1877 含 Lamigo 年份（`franchises.py` 血脈對照 `AJK011→AJL011`）。里程碑門檻由**實測單場分布**訂（非估計），投手依先發／後援分流。**順帶修好兩個既有缺陷**：`_career_pitching_per_year` 從未補本季（全站球員頁受影響）、隊史彙總只讀 `*_seasons` 導致當季破紀錄不顯示（產出的是錯的數字不只是舊的）。Coordinator 派工時要求沿用四個 canonical 生涯 helper，該指示會把王柏融的 Lamigo 安打算進台鋼，由執行者在測試中抓到並自行修正）
...
| DEV-REVIEW-PROMPT-GUARD1 | 查核提示詞產生器的三處錯誤指引 | 🏁完成 | —不適用 | [`tasks/DEV-REVIEW-PROMPT-GUARD1.md`](tasks/DEV-REVIEW-PROMPT-GUARD1.md)（T2🟡工具；**三輪 REJECT ＋ 一次 🚨已升級**，iteration 3 跨家族 APPROVE 零阻塞；merge `1155cec`。修好 `review_prompt.py` 的三處**不會報錯的錯誤指引**：①「進駐執行者 worktree」直接違反 `HANDOFF_CONTRACT` §3，改輸出 detached worktree 建立指令與自我驗證；②重現指令硬編 Python，前端卡跑 ruff/pytest 掃不到任何被審改動，改依 `main...source_sha` 判型並主動排除 `npm run lint`（未設 ESLint，會進互動式精靈）；③獨立性只依 tier 推導，忽略卡面〈查核〉欄。**③ 三次修法全被打穿**：first-match scalar 把 AND 讀成 OR → 距離式正則把條件句「跨家族查核，若失敗或有疑問再人工核可」讀成二擇一 → 格式字元直連仍被否定句與引文覆蓋，且「跨家族或人工」是「**跨家族或人工智慧**」的前綴、連詞邊界都守不住。連三退觸發升級後由需求方裁定**拆卡**：本卡改為**停止推斷**（只宣稱 tier 下限、不宣稱上限、卡面原文照登、移除全部推斷程式並加守衛測試禁其復活）——**不宣稱上限就不可能把卡面要求說低**，三輪反例全部失去適用對象；機器可讀欄位另開 `DEV-REVIEW-INDEP-FIELD1`。**方法論**：修法失敗兩次後該換的是路線不是參數；開卡時驗收條件只寫一個示範而未界定輸入空間，是三輪來回的結構成因（另開 `DOC-CARD-SPEC-RULES1`）；連三退自動升級準時觸發並強制換路線，代價有界。**本 session 期間 `main` 上一直是壞版本**，所有派工包都以 `git show <branch-sha>:scripts/review_prompt.py` 從分支借腳本產生；合併後繞道結束。`REVIEW-014` 兩筆非阻塞 nit（檔尾空白行、docstring 殘留「取較嚴者」舊措辭）依裁定併入 `DEV-REVIEW-PROMPT-GATE1` 的 rebase iteration） |
| UX-LEADERS-ORPHAN1 | LeagueLeaders 元件已無 runtime consumer | 🏁完成 | ✅已驗證 | [`tasks/UX-LEADERS-ORPHAN1.md`](tasks/UX-LEADERS-ORPHAN1.md)（T2🟦前端；獨立查核 APPROVE 零阻塞（GPT-5.6@Codex）並**由查核者代為 merge** `5537ee6`；主站 `346b033`／`3bee24c` 部署。移除 `league-leaders.tsx`（193 行）。**開卡查證推翻了 finding 的原始描述**——它不是失聯，是 `e3352e7`（`UX-GAME-HOME1`，2026-07-18）依 blueprint §3.2／§7 **刻意下架**首頁十套領先榜的遺留，功能由 `/batters`／`/pitchers` 承接，現行規格下沒有合法掛載點，故預設移除、掛載須先改規格。**留著的代價已量化**：下架後 11 天內被 `3141b71`、`c577fc8` 兩張卡改過兩次，都是對沒有使用者看得到的程式做視覺調整；查核者更報告它只能靜態審查、無法瀏覽器實測，等於讓一張卡的驗收出現產品裡不存在的死角。**差異為零以前後兩次建置的路由表逐行 diff 證明**（23 行完全相同），非口頭聲明。次要交付掃出 `matchup-card`／`lib/cols`，依卡面邊界未併入、另開 `UX-ORPHAN-SWEEP1`；掃描器第一版曾把兩支 fixtures 誤報為孤兒（測試以帶副檔名路徑引用），逐一查證時發現並修正、已自陳。**流程備查**：merge commit `5537ee6` 因訊息使用字面 `\n` 而整包成一行，`Reviewed-by` 未被 git 解析為 trailer，查核者以 `NOTE-006` 補位且未改寫已推送歷史；此暴露 canonical §6 要求 merge commit 帶 `Reviewed-by` 卻**無守衛在驗**（`test_commit_trailers.py` 明確不涵蓋 main 上的 merge commit），是否開卡待定） |
| UX-ORPHAN-SWEEP1 | 孤兒模組第二批：matchup-card 與 lib/cols（含文件對帳） | 🏁完成 | ✅已驗證 | [`tasks/UX-ORPHAN-SWEEP1.md`](tasks/UX-ORPHAN-SWEEP1.md)（T2🟦前端；**跨模型家族查核 APPROVE 零阻塞**（Gemini 3.6 Flash@Antigravity；卡面僅要求 ≠ 執行，實際超出要求）；merge `46062f3`（執行者代行，需求方授權）；主站 `346b033`／`3bee24c` 部署。移除 `matchup-card.tsx`(137)＋`lib/cols.ts`(59) 並對帳 `AI_RUNBOOK`／`UI_UX_SYSTEM`／`UI_UX_CONFORMANCE` 三份文件。**價值不在刪 196 行死碼，在止住一條假帳**：`b023385`（HYGIENE1）曾對沒有任何使用者渲染的元件做 amber token 規格對齊，而 CONFORMANCE 至今仍列 H5 未完成、第 29 列還寫 `/matchups` 使用它（`948bb21` 重寫後即不引用）——**帳面同時錯兩層**。**紅線 2 經查核者逐列確認**：H5 兩半各自交代，實測全站 `amber-[0-9]` 零命中、兩處皆由 `b023385` 修好，故記為 ✅ 已完成並註明檔案由本卡移除——未因刪檔抹掉 `standings` 那半的完成事實與出處。零引用以四種判準加匯出符號檢查獨立重驗（明確排除 `grid-cols-*` 與 `cols` prop 干擾），查核者另以獨立判準複驗一致。**Coordinator 記下一處查核宣稱落差**：檢查清單寫「21 routes 結構與大小完全相同」但證據只有單次 build、未做前後對照，該項實質證據仍是執行者的前後 diff；不影響結論，避免日後誤記） |
| UX-ENTITY-LINKS3 | 賽事情境的隊名連結化（A 層） | 🏁完成 | ✅已驗證 | [`tasks/UX-ENTITY-LINKS3.md`](tasks/UX-ENTITY-LINKS3.md)（T3🟦前端；**兩道關卡**——需求方本地人工審／Design Gate（`REVIEW-005`）＋跨家族終局查核 APPROVE 零阻塞（`REVIEW-007`）；merge `6bd387f`（執行者代行，需求方授權）；主站 `70739c9` 部署並生產驗證。**開卡證據閉環**：開卡時實測該頁 `a[href^="/teams/"]` 長度為 **0**，交付後本機與生產皆為 **4**。**兩處結構不同故取捨不同**：記分條整塊（徽章＋隊名＋戰績）可點、底線只跟文字（`ENTITY_LINK_TEXT`，沿用 `UX-ENTITY-LINKS2` 定案）；逐局比分表同列其餘格子是局數導覽 `<button>`、無外層 `<Link>`，故隊名文字自建錨點（`ENTITY_LINK`）。`TeamLogo` 兩分支改 `decorative`（§9.3），經查核者以 a11y tree 確認未重複朗讀。**B 層依 `SCOPE-002` 明確排除**（`/games` 列表與 `daily-hub` 的卡片本身即 `<Link>`，要連就得改掉整卡可點的互動語意）；生產端複驗 `/games` 102 個賽事卡連結、**0 個球隊連結**，範圍未擴張。**查核者的驗證超出交付範圍**：全 21 種 route pattern 以 runtime 代表路徑掃巢狀錨點皆 0、補上執行者因 Browser 面板白屏假象未能提供的深淺色截圖。**執行者主動揭露並被接受的限制**：gating 的 `isCurrentTeam=false` 分支在 A 層無法以真實資料觸發——記分條與逐局表只在有 livelog 時渲染（2018+），而 2018+ 出現過的隊碼全是現役 franchise（Lamigo `AJK011` 經 `franchiseOf` 映射到 `AJL011`）；改以模組層列表驗證五個已解散隊碼皆 `false`，不宣稱端到端證據。**兩筆 informational**：相依套件漏洞另開 `OPS-WEB-DEPS1`（查明四項全在建置面或未使用功能路徑，非資安事件）；Lighthouse `text-faint` 對比屬既有設計系統議題未開卡） |
| UX-ENTITY-LINKS2 | 實體連結 pattern 普及化（整塊 `hover:underline` → ENTITY_LINK） | 🏁完成 | ✅已驗證 | [`tasks/UX-ENTITY-LINKS2.md`](tasks/UX-ENTITY-LINKS2.md)（T2🟦前端；**兩道關卡**——需求方本地人工審／Design Gate（`REVIEW-007`）＋跨家族終局查核 APPROVE 零阻塞（`REVIEW-008`）；merge `0228a1c`（執行者代行，需求方明確授權，merge commit 帶 `Reviewed-by`＋`Merge-authorized-by`）；主站 `663cb14` 部署並生產驗證：`/standings` 的 `ENTITY_LINK_TEXT` group-hover 樣式 60 處、錨點最大巢狀深度 1（無 nested `<a>`）。**iteration 0 的根本錯誤**：把 §3.5「只有文字帶底線」讀成「只有文字可點」——**把視覺規範當成結構規範執行**，於是把 logo 移出 `<a>`、縮小既有可點範圍；卡面 L38 其實已寫明「logo 仍可點進同頁則保留其 wrapper，僅底線限文字」。修法是新增衍生常數 `ENTITY_LINK_TEXT`（`ENTITY_LINK` ＋ `group-hover` 變體）供外層已有 `<Link>` 的呼叫點**只套視覺不自建 `<a>`**，外層帶 `group` 讓 hover 從整塊觸發；`TeamBadge` 加 `linkStyle` prop（與 `link` 互斥）。順帶修好退休背號卡連結態的 `text-accent`——§3.5 明文禁止的實體名紅字。**Design Gate 裁定**：`RosterChips` 隊色文字被 `text-ink` 覆蓋一項需求方看過實況後接受（還原成本一行，已寫在註解）；`players/[id]` 隊名納入通過；`dynasty-chart` 不納入追認。**查核者的兩筆 informational**：交付摘要宣稱「工作區乾淨」不精確（有未追蹤的 `.claude/launch.json`）、`LeagueLeaders` 無 runtime consumer（另開 `UX-LEADERS-ORPHAN1`；查證後發現它不是失聯而是 `UX-GAME-HOME1` 依 blueprint §3.2／§7 刻意下架）。**順帶解除** `UX-ENTITY-LINKS3` 的前置阻塞——其依賴的 `ENTITY_LINK_TEXT` pattern 至此定案） |
...
| OPS-SCHEDULE-FAILURE-BLIND1 | 排程失敗沒有任何觀測面，兩個排程同日失敗三天無人知 | 🏁完成 | —不適用 | [`tasks/OPS-SCHEDULE-FAILURE-BLIND1.md`](tasks/OPS-SCHEDULE-FAILURE-BLIND1.md)（⚠️ 該檔僅 26 行、是 cutover 前的 stub；卡面實體為 [Issue #132](https://github.com/ruan6047/cpbl-analytics/issues/132) 的 165 行 body。R7 執行者與 PM 各有一次因搜該 stub 而把「08-10 21:10 即報 FAILED——當晚就報」這條**存在的**時效依據判成不存在） |
