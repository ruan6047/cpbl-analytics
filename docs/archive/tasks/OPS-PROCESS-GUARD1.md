...
- 需求：ruan6047　規劃：Claude Fable 5@Claude Code　分支：`ai/gpt-5/OPS-PROCESS-GUARD1`
- 執行：GPT-5@Codex　查核：Gemini（獨立查核 APPROVE）
- Initiative：—　spec 基線：—
- DB：`none`
- 部署：否（CI 與本機腳本層）　環境：—　PR：https://github.com/ruan6047/cpbl-analytics/pull/39　Merge SHA：`263a88a76ecc256002e5439f1d7829f9231d9e81`
...
- [x] review_prompt 對現存 33 卡：產出非 fallback 章節或 stderr 警告，二者必居其一；新增測試覆蓋標題五種變體（驗收條件／目標與驗收／驗收／驗收與回滾／Gate 與驗證）。
- [x] 章節 lint 以 pytest 形式進 CI，紅會擋 merge；生效範圍明確界定為「新 lifecycle event 後的卡」。
- [x] CI web job 跑 `npm test`；在分支上以一次故意弄紅驗證會擋（驗證後還原）。
- [x] `CLAUDE.md` 驗證清單含 web 測試。

## 驗證

- [x] `uv run ruff check`＋`uv run pytest`＋`cd web && npm test` 全綠；附 CI run 連結。
- [x] 以一張標題漂移的存量卡（如 `INGEST-GAME-TM-REFACTOR1`，僅有「Gate 與驗證」）實測 matcher 與警告路徑。
