...
- **已知的第二個消費者（僅約束 Discovery 第 3 問，不擴張本卡範圍）**：同型病在 **ops 告警**面也成立——`cpbl-check-coverage` 對大巨蛋等場館自 2026-05 起持續列出零覆蓋場次，實測至 08-03 已連續告警兩個半月無人處置（見 `INGEST-GAME-TM-REFACTOR1-G4` 卡面背景）。它**不在本卡範圍內**：那不是 CI 可機械判定的紅燈，判準、觸發點與合法去向都不同，收進來會撤掉上方非目標第四條——而那條正是 Discovery 第 1 問（會不會把所有卡鎖死）得以收斂的前提。**要求**：Discovery 第 3 問設計「去向宣告」時，須確認該機制**在概念上可推廣到非 CI 訊號**，並書面說明推廣的邊界；若不可推廣，明講並說明理由。**本卡驗收條件不因此增加任何 ops 告警的實作項**，該面另開卡套用定案後的機制。
...
- 2026-08-03 依 ruan6047 裁定登記相鄰面（見〈邊界〉末項）：ops 告警無去向為同型病的第二個表面，實例為覆蓋率檢查對大巨蛋連續告警兩個半月無人處置。**需求方原指示合併處理，經評估後改為建立連結而不合併**——理由是本卡非目標第四條刻意把範圍限縮在「CI 可機械判定的紅燈」，併入會撤掉該條、連帶鬆動 Discovery 第 1 問的收斂前提。本次只對 Discovery 第 3 問加一條「須確認機制可推廣到非 CI 訊號」的書面義務，驗收條件未變。
