...
- **識別欄合併**：隊名改為球員名前的隊徽 icon；守位（打者）/角色（投手）標籤併入球員名欄
  （stack 於名下），移除獨立欄。
- **加值指標**：新增 OPS+（打者）/ERA+（投手），league-only（沿 `trend.py` 口徑，非球場校正），
  後端 leaders API 即時補算。**二軍顯示「—」**（無單一可比聯盟基準）；**一軍當季與歷史年皆計算**，
  用「各該年」聯盟基準（年代相對、跨年代可比＝OPS+/ERA+ 的目的；1990–2025 各年 tb/hbp/sf/er/ip 實查齊全）。
  仍不混 projection、Stuff+ 未過 gate 不採。
  〔REVIEW-005 更正：初版 handoff evidence 誤述「歷史年顯示—」，經需求方 2026-07-21 裁決保留歷史值、更正驗收；程式行為本即正確。〕
