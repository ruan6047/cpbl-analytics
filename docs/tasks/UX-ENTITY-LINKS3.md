...
**✅ 需求方 2026-07-29 裁定：採 A 層，B 層不做**（見 `SCOPE-002`）。
B 層維持現狀、隊名不連——要連就得改掉「整張賽事卡可點」的既有互動語意，
以既有可用性換取隊名可點不划算。日後需求改變再另開卡，不在本卡復活。
...
- [x] ~~B 層：只出選項與取捨分析交 Design Gate~~ → **需求方已裁定不做**（2026-07-29，`SCOPE-002`）。
      `/games` 列表、`daily-hub` 等卡片本身即 `<Link>` 者維持現狀，隊名不連。
