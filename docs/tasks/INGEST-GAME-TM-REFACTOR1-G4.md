...
- [ ] **設備判準改為近期感知**：原 SQL 的 `equipped` CTE 採「**本季曾**達 0.80 覆蓋」，此前提
      ...
      **需求方已於 2026-08-04 裁定維持 10／0.80**（理由：無效重抓的成本上限為每日數個請求、
      僅在近 3 天窗口內，遠低於誤判失去自癒的代價；`0.80` 沿用既有 `COVER_OK`，避免同一概念
      在專案內出現兩個基準）。此為**需求方明示決定**，非執行者預設值；執行者不得自行更動，
      **亦不得將其描述為資料推導的結果**。修訂須另有需求方 event。
...
- [ ] **每一個 `equipped=true` 的球場各至少 1 個完成場**，kind A 與 kind D **分別**成立。
      **量綱理由**：本卡查出的失效模式是**與球場相關**的（大巨蛋整場零、亞太主早季未裝機），
      不是隨機分布於場次——因此「累計幾場」無法保證覆蓋到會壞的那一個，「每個設備場各一場」
      才對得上失效模式。此條件**自我調整**（`equipped` 集合隨資料變動），不含任何人為選定的
      整數。2026-08-03 的集合為 A 七場館（亞太主／天母／斗六／新莊／樂天桃園／洲際／澄清湖）
...
- [ ] **無賽程的逾時去向**：若某 `equipped` 球場在切換後 **14 個日曆日**內無排定主場賽事，
      不得讓 Phase B 無限等待——以 `blocked` event 記錄該球場並交需求方裁定（沿用紅線 3 的
      去向格式：具名 owner、期限、逾期維持現狀）。**14 日為需求方 2026-08-04 裁定值**
      （理由：中職一隊約每兩週輪到一次主場，14 日足以判定該球場確實排不到，21 日等於容忍
      一個半輪、讓 Phase B 卡太久）。
...
3. **`only_prod_pk` 為 0 才可進 Phase B**：正式表有、單場 API 沒有的列，**本卡不授權任何
   ...
   - 母體 **≠ 0** → **阻擋 Phase B 且阻擋本卡結案**，且**必須有指名的去向**（否則本條就是本卡
     ...
     1. 執行者產出逐筆歸因清單（官方刪球／`pitch_cnt` 重編／我方舊 bug）為 artifact。
     2. 於本卡寫 `blocked` event，**`owner` ＝ `ruan6047`**，evidence 含母體筆數、歸因分布與
        artifact 路徑，並明列解除條件。
     3. 需求方裁定期限 **7 日**（自該 `blocked` event 的 `occurred_at` 起算；需求方 2026-08-04
        裁定維持——逾期不觸發任何自動行為，故此值成本低）。
     4. 逾期未裁定 → **維持 Phase A 現狀**（新路徑續跑、flag 保留、可隨時回退）；
        **不得**自動回退、**不得**自動放行 Phase B、**不得**視為預設同意。
     5. 裁定後由需求方指定處置卡，新卡 ID 寫入本卡 Log 與後續 event。
     Gate 4 不得代為決定刪除與否，也不得以「Phase A 已通過」為由宣稱本卡完成。
...
5. **回滾觸發即回滾**：Phase A 部署後，下列任一成立即設 `CPBL_PITCH_INGEST=pitcher` 回退並
   ...
   查詢），無設備場的 0 覆蓋不是回歸：
   - **覆蓋率退步**：以**當日聚合**判定——當日新完成且 `equipped=true` 的場，
     `sum(tracked) / sum(pitches)` 低於該 kind 的**切換前 p05 地板**，且**連續兩日**成立。
     ...
     **需求方 2026-08-04 裁定的誤報容忍度：每季不超過 2 次誤報。**（定義：以歷史日聚合資料
     回放，本規則在一個球季內觸發回滾、但事後判定並無真實退步的次數。）此值**必須在看到
     回測結果之前固定**，這正是預先登錄 [pre-registration] 的要件——門檻由方法產生而非事後
     挑選。選 2 次而非某個機率值，是因為它直接對應需求方實際被打擾的次數；且誤報（白跑一次
     切換）與漏報（壞資料寫進生產）成本不對稱，**刻意偏向寧可多回滾**。
     在回測完成前，上列數值僅為**暫定值**，不得宣稱為推導結果。
   ...
   - **`equipped` 隨時間變動與地板凍結的關係**：地板凍結於切換日，但每日判定的**母體**取當日
     ...
     應該先被看見再被排除，而不是靜默排除。若某場館在切換後才恢復（或新啟用），其首次達標後
     即進入母體；**地板不因此重算**，因為地板衡量的是切換前的既有水準。
   - **母體不足時 fail closed**：切換日該 kind 的地板母體不足 **30 場**時，**不得切換該 kind**
     （2026-08-03 實測 A 187 場、D 101 場，皆遠超），須先累積或由需求方裁定。
...
- [ ] **回滾門檻的預先登錄回測（需求方 2026-08-04 裁定採降級版）**：**不做參數搜尋**，只回答
      ...
      - 誤報 **≤ 1 次／季** → **直接採用暫定值並留痕，不再調參**。
      - 誤報 **≥ 2 次／季** → 逾越裁定的容忍度，回報需求方裁定，不得自行調參。
      降級的理由：完整參數搜尋要多寫一支回測工具，而「這門檻會不會亂響」這個真正要回答的
      問題，用現值回放就足以回答。**容忍度已於裁定時固定，回放結果不得反過來修改它**
      （清單 #4 #5）。
...
- 2026-08-03 **第 2 輪卡面查核 REQUEST_CHANGES**（GPT-5／Codex）：第 1 輪六項中 F1 判 resolved，
  **F2–F6 全判 not-resolved**——共同病因是「新固定的數字仍是結果導向選參數 [selection leakage]，
  沒有事前依據」。另有三項首審缺陷。處置如下，**本則之後的定義取代上一則 Log 所述**：
  1. 〔F2〕`N=10`／`0.80` 補上選定依據並附反例：窗口下界由**已觀測最長良性中斷**決定——
     季末仍健康的球場中最長為**亞太主 8 場**（04-10～04-26，早季未裝機後恢復），窗口 ≤8 會
     讓它在四月被誤判無設備而失去自癒；10 ＝ 8＋2 餘裕。且大巨蛋連續 15 場未達標，任何 ≤15
     的窗口都會使其掉出，10 非唯一或臨界值，**不是為結果挑的**。`0.80` 為專案既有常數
     （`COVER_OK`），本卡不引入新基準。
  2. 〔F3〕回滾門檻**被實測證明是壞的**（不只是無依據）：kind D 中位 99.0% 但 **p05 僅 90.0%**、
     最低 36.6%，「中位 −10pp」＝89.0% 會讓約 5% 的正常二軍場次踩線；A 亦有單場 57.4%。
     改為**日聚合** ＋ **切換前 p05 地板**（實測 A 96.2%／D 90.0%，執行時重算）＋ **連續兩日**；
     物理欄位 mismatch 維持立即觸發。並補齊查核指出的兩個未定義：`equipped` 動態變動與凍結
     地板的關係（新失效先被看見再被排除，恢復者即時入母體但地板不重算）、母體不足 30 場時
     **不得切換該 kind**（fail closed）。
  3. 〔F4〕`only_prod_pk ≠ 0` 補上去向：`blocked` event ＋ `owner=ruan6047` ＋ **7 日裁定期限**
     ＋ 逾期維持現狀（不自動回退、不自動放行、不視為預設同意）＋ 裁定後指名處置卡 ID。
  4. 〔F5〕放行條件**改變量綱**：原「A 10 完成場／5 run、D 5 完成場／3 run」是無依據的整數；
     既然失效模式是**與球場相關**的，正確條件是**每個 `equipped=true` 球場各至少 1 完成場**
     （A、D 分別成立）。此條件自我調整、不含任何人為選定的整數，且直接對上失效模式。
  5. 〔F6〕checksum **被查核者實測打穿**（`extra_float_digits` 0 與 3 得出不同 md5）→ 指令固定
     `SET extra_float_digits = 3`；指令改為容器內可照抄（host 無 client、DB 在 5433）；
     演練範圍降級為 **`data-only recovery rehearsal`** 並**禁止再援引 `OPS-BACKUP-EMPTY1` 的 FK
     教訓**（`LIKE ... INCLUDING DEFAULTS` 不帶 PK 與三個索引），結構面另以 `pg_restore --list` 驗。
  6. 〔B3〕artifact 自證問題：payload 與 hash 同放一處無法防竄改 → 產出 `manifest.json`，
     其 sha256 寫入 handoff event evidence，錨點落在 append-only 且受分類器保護的 control-plane。
  7. 兩項 non-blocking 過度宣稱一併收斂：「設備不穩已被時序推翻」→ 資料只能證明持續無來源
     資料、不能鑑別故障或停用；kind D「與既知清單吻合」無可追溯來源 → 移除，只留查詢結果。

  查核者同時指出：`a810d82` 曾宣告 F6 已解決、而 `492e37e` 證明其指令根本不能跑，故
  **「已修訂」不得作為證據**，本輪逐項重跑。此判斷成立，後續查核請延續。
- 2026-08-04 **第 3 輪卡面查核 REQUEST_CHANGES 與需求方政策裁定**。查核（GPT-5／Codex）判
  F1／F4 resolved，F2／F3／F5／F6／B3 not-resolved。**本輪不再嘗試補推導**——三輪同一失敗
  模式已足以說明卡面撰擬階段推不出這些數字。處置分兩類：
  - **撤回不實宣稱**：F2 的「非事後挑值」撤回——「已觀測最長良性中斷 8 場」是以未來結果
    反推「良性」，屬分類政策而非無偏下界（同一定義下大巨蛋有 23 場連續區段，「季末仍健康」
    正好排除它）；`+2` 無成本依據；沿用既有常數不等於推導。F3 承認**統計單位不一致**是實質
    錯誤（p05 為單場分布卻當日聚合地板，推不出誤報率），並修正「連續兩日」與「無判定日不計」
    的衝突為 eligible day 語意。B3 宣稱收斂為「防 handoff 之後的竄改」，明寫非來源真實性錨點。
    F5 補非球場相關失效的端對端證據與無賽程逾時去向。
  - **F6 第三次同型錯誤**：卡面指令仍不能照抄（實跑的是展開後的 `docker compose exec`，寫進
    卡面的卻是 `DC="…"` 再 `$DC bash -c`，zsh 不對命令字串分詞）。改為 heredoc 並**以卡面
    字面**在 zsh 實跑通過（80,907 列、md5 相同）。**教訓入卡：卡面指令必須以卡面字面實跑，
    不得等價替換。**
- 2026-08-04 **需求方五項政策裁定**（自此為明示決定，非執行者預設值）：(1) `equipped` 維持
  窗口 10／門檻 0.80；(2) 回滾誤報容忍度 **每季不超過 2 次**，且**必須在看到回測結果前固定**；
  (3) `only_prod_pk` 裁定期限維持 **7 日**；(4) 無賽程逾時由 21 日**改為 14 日**；
  (5) 回測採**降級版**——不做參數搜尋，只回放現行暫定值的誤觸發次數，≤1 次／季即採用並留痕，
  ≥2 次則回報裁定、不得自行調參。另裁定**大巨蛋為既成資料源事實，本卡不再往下追**，
  `equipped` 判準的理由回到一般性效率性質，不得再以大巨蛋論證。
