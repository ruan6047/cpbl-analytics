...
- 部署：是　環境：production　PR：—　Merge SHA：`921de18`（source `c586b1d`；⏸未部署，待備份＋lane＋需求方 production sign-off）
- Discovery：`GAME-RECAP-DATA1` Checkpoint 1 已核可
- Design：Design Gate N/A；純持久化基礎設施，無 public UI
- current-state：📦已合併／⏸未部署（2026-07-24，REVIEW-004/MERGE-005）。交付 `migrations/066_game_recap_pa_expand.sql`（因 main 平行落 `065_game_tm_shadow` 改號 065→066，DDL 不變）+ 7 守門測試；跨家族 APPROVE。**production 部署預備見執行 worktree `_review_artifacts/PROD_DEPLOY_RUNBOOK.md`**；部署驗證後解鎖 BUILD1／INGEST-RECORDS-HR1。
...
- [~] migration rehearsal、rollback／restore 路徑與 query plan 證據完整（✅ 已於隔離 DB 重現）；**production 前取得備份、schema lane 與需求方 sign-off（⏸ 部署時完成）**。
