...
> 用視覺化與數據分析，把中華職棒 [CPBL] 的比賽、球員與歷史看得更懂——賽況復盤、進階數據與賽前勝率。

結構與 hero 副標一致（手段 → 目的 → 範圍），並補回「中華職棒」與 `CPBL` 兩組關鍵字。

**長度刻意控制在行動版 SERP 截斷點（約 40–50 中文字）內**：初版尾端還有「，非官方獨立專案」共 59 字，行動版搜尋結果多半看不到那一段，需求方 2026-08-03 人工審時裁定移除。免責已在 footer 完整呈現，不需佔用 description 的字數。
...
- 2026-08-03 需求方人工審 diff 後裁定**縮短首頁 description**：移除尾端「，非官方獨立專案」（59 字 → 51 字），理由是行動版 SERP 約在 40–50 中文字截斷，該段落實際上看不到，而免責已在 footer 完整呈現。此為文案長度取捨，不影響關鍵字覆蓋——「中華職棒」與 `CPBL` 皆在前半段。
