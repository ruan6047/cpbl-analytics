...
- [ ] **H9**（資料擴充，非缺陷；Design Gate 已 sign-off §9.6 五階 2026-07-24）：擴 `TeamMeta` 加 `name3`（3–4 字名，如 統一獅）、`char1`（中文 1 字，龍/獅/象/邦/猿/鷹，**僅供文字階；⑤ICON 維持英文 `letter` 不改**）以支援 §9.6 ②④；③二字階：將中信兄弟 `short` 由「兄弟」改「中信」（母企業）或加專欄。驗收：六隊 `name3`/`char1` 齊備、③二字一致、五階皆可渲染。
