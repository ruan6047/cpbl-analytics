...
全庫共 **296 對 island** 屬此型態（2018–2026 每年皆有，2022 後每年 50–65 例）。
...
（＝該事件後的累計出局數）重建每半局 running outs 當權威值對照：修正前有真實投球的 island
**330,386 個中 2,157 個（0.653%）**不一致，差值 `-1` 有 2,148 例、`-2` 有 7 例、`+2` 有 2 例
——幾乎全是來源欄位慢一拍。推導值本身可信：71,023 個半局收在恰好 3 出局。

## 離線驗證（開卡前已跑，用真實資料，未寫入任何東西）

合併判準（**iteration 2 修正後**）：同半局的打者變化**必須**兩段同一 `batting_order`
（＝該半局第幾位打者，代打接替不另開棒次槽），再加上「球數未歸零續投」或
「`更換代打` 公告列」其一佐證，才視為同一打席。
...
**iteration 1 的判準有 Critical 缺陷（查核抓到，已修）**：當時「球數續投」不檢查
`batting_order`，而「新打席首列球數必 ≤1」這個前提**實測為假**——全庫有 **7 個真打席邊界**
的來源球數不歸零（`2021/D/64` 6 局下棒次 5 於 1-1 結束、棒次 6 首列是 2-1；`2018/A/4` 棒次
11 的 (1,0) 接棒次 12 的 (2,0)；另 `2018/A/33`、`2018/D/6`、`2018/D/73`、`2020/A/90`、
`2020/D/21`），會被合併成一個打席。修正後 303 → **296**，該 7 對全數排除且有回歸測試。

另一個設計邊界：較寬鬆的規則會把「零投球故意四壞（`故意四壞球上壘。`）＋緊接著的打席間
代打」誤併（如 `2018/A/9` 9 局上、`2018/A/16` 8 局下）。**兩個邊界執行者與查核者都必須確認。**
...
1. **合併規則只能合併同一打席，不得合併兩個真打席。** 具體門檻：`2018/A/9` 9 局上、
   `2018/A/16` 8 局下（零投球故意四壞 → 打席間代打），以及 iteration 1 被查核抓到的
   7 對球數不歸零邊界（`2021/D/64`、`2018/A/4`、`2018/A/33`、`2018/D/6`、`2018/D/73`、
   `2020/A/90`、`2020/D/21`），全部必須維持為**兩個** PA；合併對數須逐對可列舉並附產生腳本。
   **球數單獨不構成續打席證據**——該前提已被實證推翻，棒次槽相同是必要條件。〔清單 #8〕
...
看到的是既有欄位的值變正確；`end_hitter_acnt` 為**新增**欄位（migration 068，nullable），
既有消費端不需改動即可繼續運作。
...
- 2026-07-29 iteration 1 查核 REJECT（1 Critical + 2 Major）：`count_continues` 未檢查
  ...
  9.15(b) 打席歸屬與 `end_hitter_acnt` 欄。
