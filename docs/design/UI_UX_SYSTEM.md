...
- **為何實體名不用 accent 紅**：`accent`（#d12638）同時是行動色＋數據差(down)色，紅字實體名潛意識讀成負面／行動；改沉穩 `text-ink` ＋ 常駐底線——**連結性以底線（非色彩單獨）可辨識**（WCAG 1.4.1）。
...
- **已驗證（`UX-TOKEN-HYGIENE1` H2/H3，2026-07-24）**：`--chart-1..8` 以 `dataviz` `validate_palette.js` 驗——**淺色相鄰對最差 ΔE 10.1、深色 9.5（皆過 ≥8 門檻）**，色盲可辨識；`chart-6`(#94a3b8) 為刻意中性灰槽（chroma-floor 旗標非缺陷）；少數低對比槽（淺色 amber/teal、灰）對白底 <3:1，靠 recharts 圖例/表格檢視豁免（validator 允許）。深色原缺的 `chart-7/8` 已補（`7=#db2777` 沿用以免與灰 `chart-6` 在 deutan 下相撞、`8=#e08a2a` 提亮以齊 dark 色階；CVD 與 navy 對比通過）。故**未重新配色**，僅補值+記錄。
...
由長到短五階，依可用空間降階（動因：**統一7-ELEVEn獅** 9 字中英混排會撐破窄欄/chip，違反 375px 鐵則）。**判準：容器會換行/溢出就降一階；固定寬表格欄禁塞全稱。**

| 階 | 形式 | 統一 | 味全 | 中信兄弟 | 富邦 | 樂天 | 台鋼 | 資料狀態 |
|---|---|---|---|---|---|---|---|---|
| ① **全稱** | 官方全名 | 統一7-ELEVEn獅 | 味全龍 | 中信兄弟 | 富邦悍將 | 樂天桃猿 | 台鋼雄鷹 | ✅ `teamFullName()` |
| ② **三～四字名** | 隊＋吉祥物（去贊助商雜訊） | 統一獅 | 味全龍 | 中信兄弟 | 富邦悍將 | 樂天桃猿 | 台鋼雄鷹 | ✅ `teamName3()` |
| ③ **二字簡稱** | 隊慣稱（現有 `short`） | 統一 | 味全 | 兄弟 | 富邦 | 樂天 | 台鋼 | ✅ `teamShort()` |
| ④ **一字簡稱** | 吉祥物/識別單字 | 獅 | 龍 | 象 | 邦 | 猿 | 鷹 | ✅ `teamChar1()` |
| ⑤ **ICON（隊徽）** | 隊色方塊＋字 | 〔L〕 | 〔W〕 | 〔B〕 | 〔G〕 | 〔R〕 | 〔T〕 | ✅ `LetterBadge` |

- **資料（`UX-TOKEN-HYGIENE1` H9 已實作）**：`lib/teams.ts` 五階皆就緒——①`teamFullName()`、②`teamName3()`（新欄 `name3`）、③`teamShort()`（現有 `short`）、④`teamChar1()`（新欄 `char1`）、⑤`LetterBadge`（`letter`）。**③二字經 grilling 定案沿用現有 `short`（中信兄弟＝「兄弟」隊慣稱），不改為母企業「中信」**——因 `short` 已用於大量球迷向文案（box-tabs 分頁、亮點「兄弟 煮粥」等），改之會使文案變生硬。②④欄已加、accessor 就緒，實際渲染留給有窄空間需求的卡。
- **Design Gate 定案（2026-07-24）**：
  - **② 三～四字名值確認**：統一獅／味全龍／中信兄弟／富邦悍將／樂天桃猿／台鋼雄鷹。
  - **⑤ ICON 保留、字用英文字母（W/B/L/G/R/T，方案 b）**：ICON 價值在「隊色＋字」靠顏色掃描辨隊；⑤走英文（緊湊、與官方字母標一致）、④中文一字並行，**各司其職**。`LetterBadge` 不需改。
