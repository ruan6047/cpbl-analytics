...
| ③ **二字簡稱** | 母企業/贊助商 | 統一 | 味全 | 中信 | 富邦 | 樂天 | 台鋼 | ⚠️ `short` 大致有＊ |
| ④ **一字簡稱** | 吉祥物/識別單字 | 獅 | 龍 | 象 | 邦 | 猿 | 鷹 | ⚠️ 需新欄 |
| ⑤ **ICON（隊徽）** | 隊色方塊＋字 | 〔L〕 | 〔W〕 | 〔B〕 | 〔G〕 | 〔R〕 | 〔T〕 | ✅ `LetterBadge` |

- **資料現況（誠實，不腦補）**：`lib/teams.ts` 現有 ①全名（`teamFullName`）、③的 2 字 `short`、⑤英文 `letter`（W/B/L/G/R/T）。**＊但 `short` 於中信兄弟現為「兄弟」，本階梯 ③ 要「中信」（母企業）**——須改 `short` 或加欄。**②的 3–4 字名、④的中文 1 字皆無欄位**。實作此階梯須擴 `TeamMeta`（加 `name3`/`char1`，並定 ③ 用 `short` 或改「中信」）——屬資料/改碼任務（→ `UX-TOKEN-HYGIENE1` H9），非現況。
