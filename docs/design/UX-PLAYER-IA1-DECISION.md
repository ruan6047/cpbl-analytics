...
> 狀態：**已核可（2026-07-20，需求方 ruan6047）**——變體採 **C・Hybrid**；§1 骨架／角色規則／狀態契約與 §2 遷移 map 全數凍結。UX-PLAYER-SECTIONS1 依此實作，不得再改層結構。
...
## 5. 需求方決策（2026-07-20 裁決）

- [x] **變體**：**C・Hybrid**（總覽常駐＋下方 sticky 三分頁；deep-link 走 `?sec=` query）——採 §4 建議
- [x] **遷移 map**（§2）核可，模組安置照表實作，無調整
- [x] 凍結項（§1 骨架、角色規則、狀態契約）確認

> 裁決者：ruan6047　時機：UX-PLAYER-IA1 已 🏁完成（merge `37825e1`）後補齊本文件缺漏的變體擇一；
> REVIEW-014 的 APPROVE 核可的是交付品質，變體選擇於本次補記。
> 本裁決是 UX-PLAYER-SECTIONS1 的實作基線，該卡不得再改層結構。
