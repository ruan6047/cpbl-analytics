...
全庫 `scripts/<name>.<ext>` 字面路徑共 **946** 處。**只有 `enforced` 那一面強制**——其餘的過期是歷史事實不是缺陷：
...
| `enforced` | 172 | ✅ 強制 | `scripts/`、`src/`、活契約與設計文件——壞了就是現在的缺陷 |
| `historical` | 145 | 回報 | `docs/archive/**` 與卡片交付產物＝凍結證據，**本卡射程外** |
