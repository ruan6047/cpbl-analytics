...
全庫 `scripts/<name>.<ext>` 字面路徑共 **952** 處。**只有 `enforced` 那一面強制**——其餘的過期是歷史事實不是缺陷：
...
| `enforced` | 176 | ✅ 強制 | `scripts/`、`src/`、活契約與設計文件——壞了就是現在的缺陷 |
| `historical` | 147 | 回報 | `docs/archive/**` 與卡片交付產物＝凍結證據，**本卡射程外** |
| `fixture` | 36 | 不適用 | `tests/**` 的合成路徑；真實路徑壞掉 pytest 自己會紅（更強的機制） |
...
| `measure_notification_delivery.py` | 一次性產物 | `docs/research/OPS-SCHEDULE-FAILURE-BLIND1/` | ✅ | 唯讀 | ✅ --help 安全 | OPS-SCHEDULE-FAILURE-BLIND1 | macOS 通知投遞的可重建量測（OPS-SCHEDULE-FAILURE-BLIND1／#132）。 | ⚠️ 未查證 |
