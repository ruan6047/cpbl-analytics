...
def _calibration_slope_null(
    ...
    **條件零假設**：「給定 p̂，各場賽果彼此獨立」。5% 的型一誤差只在這個條件下成立。
    同週賽果若有殘餘相依，效果的**方向取決於相依的正負**：正向群聚會讓真實零分布比
    模擬的寬，IID 區間因此偏窄、實際型一誤差高於名目 5%；負向相依則相反，零分布較窄、
    IID 區間偏寬、型一誤差低於名目值。**本卡未估計該相依的方向與大小**，故不得宣稱這個
    檢定在任一邊偏保守，只能說它是條件零假設下的名目 5% 檢定。
