...
1. **逐球缺漏**：完成場、球場有 TrackMan 設備、但 pitch_tracking 覆蓋率 < 門檻。
   設備判定為經驗式——某球場只要**本季曾有**任一場覆蓋率 ≥ 80% 即視為「有設備」。
   缺漏場印出建議：重跑 cpbl-scrape-pitches。

   ⚠️ **此處的「本季曾達」判準是 recency-blind 的，其原始理由已被實測推翻**
   （2026-08-03，見 docs/tasks/INGEST-GAME-TM-REFACTOR1-G4.md〈背景〉）：舊註解寫
   「避免把大巨蛋等無設備場誤報」，但大巨蛋並非無設備場——該場 2026-03 覆蓋率 98.4%，
   自 06-02 起才連續全零。設備狀態是**時變**的，用全季歷史當判準會讓已停止產出的球場
   永久通過測試。refresh 鏈（`run_refresh_recent._lagging_pitch_games`）已於
   INGEST-GAME-TM-REFACTOR1-G4 改為「最近 10 場至少一場達 0.80」的近期感知判準；
   **本工具的判準與告警去向刻意未在該卡調整**（缺陷在告警無強制去向而非偵測失靈），
   屬另卡範圍。此處保留舊行為是已知且刻意的，不是遺漏。
...
def main() -> None:
    ...
    if zero:
        # 措辭紅線：資料只能證明「該場源頭持續無 TrackMan」，**不足以鑑別**硬體故障／停用／
        # 其他原因。原註記「大巨蛋設備不穩」屬過度推論（06-02 起連續全零已非偶發不穩），已收斂。
        print(f"\nℹ️  逐球完全零 {len(zero)} 場（設備場但整場無逐球；重爬後仍零＝該場源頭無 TrackMan，非缺漏。"
              f"含亞太主早季未裝機、大巨蛋自 2026-06-02 起持續無來源資料）：")
        for r in zero:
