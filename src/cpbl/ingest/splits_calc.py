...
VENUE_OFFICIAL = {
    ...
    # 歷史別名（同座球場的舊短名／變體，官方生涯分項同一 F 碼）：
    # 桃園＝桃園國際棒球場 2018–2021 場名（F23，冠名後為樂天桃園）；
    # 亞太副場＝亞太副 2024–2025 標籤變體（F30）
    "桃園": "樂天桃園棒球場",
    "亞太副場": "臺南亞太國際棒球訓練中心成棒副球場",
}
...
PA_OUTCOME: dict[str, dict[str, int]] = {
    ...
    "雙礙守": {"pa": 1, "ab": 1, "go": 1},
    "三呎線": {"pa": 1, "ab": 1, "go": 1},
    # ── 2018–2025 歷史詞彙（VENUE-PARK1 回填時逐詞抽 content 原文定案，共 14 詞）──
    "三殺": _GO,                            # 內野滾地三殺守備（同雙殺歸 go）
    "中滾": _GO, "左滾": _GO, "右滾": _GO,   # X外野滾地球出局/趁傳（滾地型非安打）
    "內二": _H2,                            # 「二壘內野安打」＝安打
    "場三": _H3,                            # 「三壘場地安打」（同場二/場安家族）
    "投": _GO, "捕": _GO,                   # 觸擊滾地出局/趁傳，截斷詞（同投滾/捕滾）
    "飛": _FO,                              # 「打者-飛球接殺出局」截斷詞
    "雙誤": _GO,                            # 「雙殺打上壘-失誤上壘」：雙殺＋內野失誤皆 go
    "觸傳球": {"pa": 1, "ab": 1, "go": 1},  # 三呎區外遭傳球觸及（同三呎線語意）
    "觸球": {"pa": 1, "ab": 1, "go": 1},    # 擊球員碰觸界內球出局（內野滾地；同礙守）
    "犧飛誤": {"pa": 1, "sf": 1, "fo": 1},  # 犧牲飛球＋野手失誤上壘（同犧短誤 pattern）
    # #207：2026/A/347「犧牲飛球上壘-趁傳」；PA/SF/不計打數對過官方投打 box，
    # fo 沿用犧飛/犧飛誤慣例（非官方逐場 FO 證明）
    "犧飛趁": {"pa": 1, "sf": 1, "fo": 1},
    # 型態不明不歸滾飛（同「違規」處理；占比極低，go/fo 缺口記於驗證報告）：
    "失": {"pa": 1, "ab": 1},               # 失誤上壘但無守位前綴（滾/飛逐例不一，不猜方向）
    "裁決": {"pa": 1, "ab": 1},             # 促請裁決/打序錯誤出局（擊球結果被判決取代）
}
...
def calc_pitching_t1(year: int, kind: str, cutoff: dict[str, date]) -> tuple[Table, Table]:
    """回傳 (splits 家族 1/4/9/10, vs各隊)。save_ok 由官方救援成功投手判定（`official_closer_sql`：
    旗標 is_save_ok 優先、再退 games.closer_id——該欄單獨用會漏 2026-A-99／229／313）。
    ...
    sql = f"""
        SELECT pg.pitcher_acnt, pg.visiting_home_type, g.game_date, g.venue,
               g.home_team_name, g.away_team_name, {official_closer_sql("g")} AS closer_id,
