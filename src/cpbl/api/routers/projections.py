...
@router.get("/api/v1/projections/pitching")
def pitching_projections(
    year: int | None = Query(None, description="目標季；省略則取最新一版的最大目標季"),
    stat: str = Query("era", pattern="^(era|whip|k9|bb9)$"),
    limit: int = Query(25, ge=1, le=200),
) -> dict:
    """最新模型版本的投手投影排行（Marcel；era/whip/bb9 低為佳升冪、k9 降冪）。"""
    asc = stat in ("era", "whip", "bb9")
    with conn() as c:
        cur = c.cursor()
        cur.execute("SELECT id FROM cpbl.model_versions WHERE task = 'pitching_projection' "
        ...
        if year is None:
            cur.execute(
                "SELECT max(target_year) FROM cpbl.projections "
                "WHERE model_version = %s AND actual IS NULL",
                (model_version,),
            )
            r = cur.fetchone()
            year = r[0] if r and r[0] else None
        ...
        cur.execute(
            ...
            JOIN cpbl.players p ON p.id = pr.player_id
            WHERE pr.model_version = %s AND pr.stat = %s AND pr.target_year = %s
            ORDER BY pr.predicted {'ASC' if asc else 'DESC'}
            LIMIT %s
            """,  # noqa: S608 — 排序方向為內部布林
            (model_version, stat, year, limit),
        )
