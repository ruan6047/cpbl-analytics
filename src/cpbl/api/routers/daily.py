...
def _unresolved_statuses(cursor, rows: list[dict]) -> dict[tuple[int, str, int], str]:
    ...
    這一格取代了原本硬編的 `"unknown"`。**值得多這一次查詢的理由是它真的分得開三件事**
    ——`cpbl.games` 一律只看得到 0–0：

    - `postponed`／`reserved`：官方已宣告延賽／保留，資料沒有落後，沒有人需要做事；
    - `scheduled`：官方那邊也還沒有結果（開賽前、或官網自己還沒更新）；
    - `final`：**官方說打完了、我們卻還是 0–0**——這才是真的刷新落後，唯一要人去看的一格。

    `rows` 為空時完全不查（休兵日與絕大多數日子的常態就是 0 筆），所以首頁的查詢數只在真的
    有未定案場次時才 +1。表尚未 migrate／權限不足時整批退回 `unknown`，不讓首頁掛掉（同
    `_last_refresh` 的立場：缺證據就別宣稱）。
    """
    if not rows:
        return {}
    ...
    except Exception:  # noqa: BLE001 — 缺表／權限問題退回 unknown，與原本硬編時的行為相同
        return {}
...
def daily_summary(
    ...
    return {
        ...
        "freshness": {
            ...
            # 過去日期卻仍無比分。`cpbl.games` 只看得到 0–0，分不出刷新落後與延賽未更新新
            # 日期，所以 status 改引用**官方排程狀態**（`_unresolved_statuses`，與單場狀態
            # 端點共用同一份判定）——這是 API-DAILY-SUMMARY1 當初登記、等官方狀態語意定案
            # 後要結清的串接義務。官方狀態也讀不出來時才退回 unknown（fail closed）。
            "unresolved_games": [
