...
from cpbl.api.unresolved import UNRESOLVED_WINDOW_DAYS, statuses_for
...
def daily_summary(
    ...
    （常態是 0 筆故不查，見 `unresolved.statuses_for`）。今日場次另加最多 3 次 Redis GET（唯讀
    ...
    with conn() as c:
        ...
        unresolved_status = statuses_for(cur, unresolved)
    ...
    return {
        ...
        "freshness": {
            ...
            # 新日期，所以 status 改引用**官方排程狀態**（`unresolved.statuses_for`，與單場
